...
export async function GET(request: NextRequest) {
  ...
  if (scan === "true" && cdeId) {
    try {
      ...
      console.log(
        `[AutoMatch Scan] Found ${deals?.length || 0} available NMTC deals for CDE ${(cdeData as Record<string, unknown>).name}`,
      );
      ...
      const matches = dealsRaw
        .map((deal) => {
          ...
          return {
            ...
            programType: (((deal.programs as string[]) || [])[0] || "NMTC") as
              | "NMTC"
              | "HTC"
              | "LIHTC",
            ...
            submittedDate:
              deal.submitted_at || deal.created_at || new Date().toISOString(),
          };
        })
      ...
      console.log(
        `[AutoMatch Scan] Returning ${matches.length} matches above ${minScore}%`,
      );
  ...
  if (!dealId) {
    return NextResponse.json(
      { error: "dealId or scan+cdeId required" },
      { status: 400 },
    );
  }
...
function calculateDealCdeMatch(
  deal: Record<string, unknown>,
  cde: Record<string, unknown>,
  ...
  const isNational =
    ...
    (marketText ? /\bnational\b/.test(marketText) : false);
  ...
  const isRural =
    intakeData.isRural ||
    ((deal.tract_classification || "") as string)
      .toLowerCase()
      .includes("rural");
  ...
  const dealSector = normalizeText(
    (intakeData.sectorCategory || deal.project_type) as string,
  );
  const cdeMarket = normalizeText(cde.predominant_market as string);
  const cdeSectors = ((cde.target_sectors || []) as string[]).map(
    normalizeText,
  );
  ...
  const minDistress =
    Number(cde.min_distress_percentile || cde.min_distress_score) || 0;
  if (minDistress === 0) {
  ...
  const isUts =
    isDealInUnderservedState(dealStateForUts, cdeYear) ||
    intakeData.isUts ||
    false;
  ...
  // 11. Entity Type
  const orgType = normalizeText(
    (intakeData.organizationType || intakeData.entityType) as string,
  );
  const isNonProfit =
    orgType.includes("nonprofit") ||
    orgType.includes("non-profit") ||
    orgType.includes("501");
  ...
  if (cdeNonprofitPreferred && isNonProfit)
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (accessToken) {
      ...
      if (resp.status !== 401 && resp.status !== 403) {
        return NextResponse.json(data || { error: "AutoMatch failed" }, {
          status: resp.status,
        });
      }
    }
    ...
    console.log(
      "[AutoMatch POST] Backend unavailable, using local scoring engine",
    );
  }
  ...
  try {
    ...
    for (const cde of cdes || []) {
      ...
      const score = calculateDealCdeMatch(
        deal as Record<string, unknown>,
        enrichedCde,
      );
      const orgId = String(
        (cde as Record<string, unknown>).organization_id ||
          (cde as Record<string, unknown>).id,
      );
      ...
      if (!existing || score.total > existing.totalScore) {
        ...
        orgBestMatch.set(orgId, {
          ...
          matchStrength:
            score.total >= MATCH_THRESHOLDS.excellent
              ? "excellent"
              : score.total >= MATCH_THRESHOLDS.good
                ? "good"
                : score.total >= MATCH_THRESHOLDS.fair
                  ? "fair"
                  : "weak",
    ...
    return NextResponse.json(
      { error: "AutoMatch scoring failed" },
      { status: 500 },
    );
  }
}
