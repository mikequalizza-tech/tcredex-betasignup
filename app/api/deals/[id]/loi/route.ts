...
export async function POST(
  ...
  try {
    ...
    const cdeId = user.organizationType === 'cde'
    ...
    try {
      ...
      const [{ data: deal }, { data: cde }] = await Promise.all([
        ...
        supabase
          .from('cdes')
          .select('organization_name, parent_organization, primary_contact_name')
          .eq('id', cdeId)
          .single(),
      ]);
      ...
      if (deal) {
        ...
        const cdeName =
          (cde as { organization_name?: string | null; parent_organization?: string | null; primary_contact_name?: string | null } | null)?.organization_name
          || (cde as { organization_name?: string | null; parent_organization?: string | null; primary_contact_name?: string | null } | null)?.parent_organization
          || (cde as { organization_name?: string | null; parent_organization?: string | null; primary_contact_name?: string | null } | null)?.primary_contact_name
          || 'A CDE';
