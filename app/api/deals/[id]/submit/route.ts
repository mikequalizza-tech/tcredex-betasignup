  ...
  try {
    ...
    if (user.organizationType !== "sponsor") {
      return NextResponse.json(
        { error: "Only sponsors can submit deals" },
        { status: 403 },
      );
    }
    ...
    const mergedData: Record<string, unknown> = {
      ...
      address:
        rawIntake.address ||
        dealRow.address ||
        (dealRow.city ? `${dealRow.city}, ${dealRow.state}` : undefined),
      programs: rawIntake.programs || dealRow.programs,
      totalProjectCost:
        rawIntake.totalProjectCost || dealRow.total_project_cost,
      // Tier 2: Impact, Benefits, Team, Capital, Site, Timeline
      communityImpact:
        rawIntake.communityImpact ||
        rawIntake.communitySupport ||
        dealRow.community_benefit,
      communitySupport:
        rawIntake.communitySupport ||
        rawIntake.communityImpact ||
        dealRow.community_benefit,
      permanentJobsFTE:
        rawIntake.permanentJobsFTE ??
        dealRow.permanent_jobs_fte ??
        dealRow.jobs_created,
      constructionJobsFTE:
        rawIntake.constructionJobsFTE ?? dealRow.construction_jobs_fte,
      siteControl: rawIntake.siteControl || dealRow.site_control,
      constructionStartDate:
        rawIntake.constructionStartDate || dealRow.construction_start_date,
      targetClosingDate:
        rawIntake.targetClosingDate || dealRow.projected_closing_date,
      // Tier 3: Due Diligence
      phaseIEnvironmental:
        rawIntake.phaseIEnvironmental || dealRow.phase_i_environmental,
