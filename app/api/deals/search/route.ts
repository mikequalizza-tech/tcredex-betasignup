...
async function enrichDealsWithSponsorNames(supabase: ReturnType<typeof createServerClient>, deals: Array<Record<string, unknown>>) {
  ...
  const sponsorIds = Array.from(
    new Set(deals.map((deal) => deal.sponsor_id).filter((value): value is string => typeof value === 'string'))
  );
