    ...
    if (user.organizationType === "sponsor") {
      ...
      if (investorData) {
        ...
        if (!commitmentError && commitmentRows && commitmentRows.length > 0) {
          const committedDealIds = Array.from(
            new Set(
              commitmentRows
                .map((row: Record<string, unknown>) => row.deal_id as string)
                .filter(Boolean),
            ),
          );
          const existingIds = new Set(
            (deals || []).map((d: Record<string, unknown>) => d.id as string),
          );
          const missingDealIds = committedDealIds.filter(
            (dealId: string) => !existingIds.has(dealId),
          );

          if (missingDealIds.length > 0) {
            const { data: committedDeals, error: committedDealsError } =
              await supabase
                .from("deals")
                .select(
                  `
