...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const tractLookupUrl = new URL('/api/geo/tract-lookup', request.nextUrl.origin);
    ...
    const tractLookupData = (await tractLookupResponse.json().catch(() => null)) as TractLookupPayload | null;
    ...
    const geoid = normalizeGeoid(tractLookupData.tract_id || tractLookupData.geoid);
    if (!geoid) {
      return NextResponse.json(
    ...
    return NextResponse.json({
      ...
      has_any_tax_credit: tractLookupData.has_any_tax_credit ?? tractLookupData.eligible ?? false,
