...
export async function GET(request: NextRequest) {
  try {
    ...
    const roleTable = user.organizationType === 'sponsor'
      ? 'sponsors'
    ...
    if (roleTable) {
      ...
      if (!orgError && orgData) {
        return NextResponse.json({
          organization: {
            id: id,
            name: orgData.name || orgData.organization_name || orgData.primary_contact_name || 'Organization',
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (primary_contact_phone !== undefined) updateData.primary_contact_phone = primary_contact_phone;
    if (phone !== undefined) updateData.primary_contact_phone = phone;
    if (address !== undefined || address_line1 !== undefined) updateData.address = address || address_line1;
    ...
    if (user.organizationType === 'sponsor') {
      ...
      if (low_income_owned !== undefined) updateData.low_income_owned = low_income_owned;
      if (woman_owned !== undefined) updateData.woman_owned = woman_owned;
      if (minority_owned !== undefined) updateData.minority_owned = minority_owned;
    ...
    if (user.organizationType === 'cde') {
      ...
      if (year_founded !== undefined) updateData.year_established = year_founded;
    ...
    if (name !== undefined) {
      console.log('[Organization] Syncing name to users table:', { organization_id: id, name });
      const { data: usersData, error: usersError, count: _count } = await supabase
        .from('users')
        .update({ organization_name: name, updated_at: new Date().toISOString() })
        .eq('organization_id', id)
        .select('id, email, organization_name');

      if (usersError) {
        console.warn('[Organization] Failed to update users table:', usersError);
        // Don't fail the request, role table was updated successfully
      } else {
        console.log('[Organization] Users table updated:', { rowsUpdated: usersData?.length || 0, users: usersData });
      }
    }
    ...
    return NextResponse.json({
      organization: {
        id: id,
        name: updatedOrg.name || updatedOrg.organization_name || updatedOrg.primary_contact_name || 'Organization',
        type: user.organizationType,
        ...updatedOrg,
      },
      success: true
