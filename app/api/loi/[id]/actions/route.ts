    ...
    switch (action) {
      case "issue":
        if (
          ...
          !(await canUserManageLoiAsCde(user.organizationId, existing.cde_id))
        ) {
          return NextResponse.json(
            {
              success: false,
              error: "Only the issuing CDE can issue this LOI",
            },
            { status: 403 },
          );
        }
      ...
      case "respond":
        if (!body.response) {
          return NextResponse.json(
            {
              success: false,
              error: "response object required for respond action",
            },
            { status: 400 },
          );
        }
        if (
          ...
          return NextResponse.json(
            {
              success: false,
              error: "Only the sponsor can respond to this LOI",
            },
            { status: 403 },
          );
        }
        loi = await service.sponsorRespond(
          id,
          body.response,
          existing.sponsor_id,
        );
      ...
      case "withdraw":
        ...
        if (
          ...
          return NextResponse.json(
            {
              success: false,
              error: "Only the issuing CDE can withdraw this LOI",
            },
            { status: 403 },
          );
        }
...
async function canUserManageLoiAsCde(
...
async function canUserRespondAsSponsor(
  userOrganizationId: string,
  sponsorId: string,
): Promise<boolean> {
  const supabase = getSupabaseAdmin();
