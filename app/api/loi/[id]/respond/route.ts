...
export async function POST(
  ...
  try {
    ...
    if (
      ...
      !(await canUserRespondAsSponsor(user.organizationId, loi.sponsor_id))
    ) {
      return NextResponse.json(
        {
          success: false,
          error: "You are not authorized to respond to this LOI",
        },
        { status: 403 },
      );
    }
    ...
    if (loi.status !== "pending_sponsor") {
      return NextResponse.json(
        {
          success: false,
          error: `LOI is not pending sponsor response (current status: ${loi.status})`,
        },
        { status: 400 },
      );
    }
