...
import { AuthError, AuthUser, handleAuthError, requireAuth, verifyDealAccess } from '@/lib/api/auth-middleware';
...
async function assertDocumentAccess(
  ...
  if (doc.deal_id) {
    await verifyDealAccess(request, user, doc.deal_id, mode === 'edit' ? 'edit' : 'view');
    return;
  }
