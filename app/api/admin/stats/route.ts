...
export async function GET(request: NextRequest) {
  try {
    ...
    const [userCount, sponsorCount, cdeCount, investorCount, dealsResult] = await Promise.all([
      ...
      supabase
        .from('deals')
        .select('id, status, created_at, updated_at, programs, nmtc_financing_requested')
    ...
    const totalAllocation = deals
      .filter((deal) => ['available', 'seeking_capital', 'matched', 'closing', 'closed'].includes(deal.status || ''))
    ...
    const orgCount = {
      count: (sponsorCount.count || 0) + (cdeCount.count || 0) + (investorCount.count || 0),
    };
