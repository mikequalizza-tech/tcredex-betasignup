  ...
  try {
    ...
    switch (type) {
      ...
      case 'allocation_request': {
        // Send Allocation Request — Sponsor → CDE (flagship outreach email)
        const {
          to, contactName, cdeName, cdeAllocationAmount, cdeAllocationYear,
          sponsorName, sponsorContactName, projectName, dealSummary,
          claimUrl, dealId,
        } = body;

        if (!to || !contactName || !cdeName || !sponsorName || !sponsorContactName || !projectName || !dealSummary || !claimUrl || !dealId) {
          return NextResponse.json(
            { error: 'Missing required fields: to, contactName, cdeName, sponsorName, sponsorContactName, projectName, dealSummary, claimUrl, dealId' },
            { status: 400 }
          );
        }

        const result = await email.allocationRequest(
          to,
          contactName,
          cdeName,
          cdeAllocationAmount || '',
          cdeAllocationYear || new Date().getFullYear(),
          sponsorName,
          sponsorContactName,
          projectName,
          dealSummary,
      ...
      default:
        return NextResponse.json(
          { error: 'Invalid email type. Use: allocation_announcement, profile_added, allocation_request, bulk_allocation, bulk_profile_added' },
          { status: 400 }
        );
    }
  } catch (error) {
