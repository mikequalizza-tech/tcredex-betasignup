...
export async function POST(request: NextRequest) {
  try {
    ...
    const unwrapBackendData = <T = Record<string, unknown>>(
      payload: unknown,
    ): T => {
      if (
        payload &&
    ...
    switch (action) {
      case "batch": {
        ...
        if (!resp.ok) {
          return NextResponse.json(data || { error: "Batch matching failed" }, {
            status: resp.status,
          });
        }
      ...
      case "single": {
        ...
        const data = await resp.json().catch(() => ({}));
        if (!resp.ok) {
          return NextResponse.json(data || { error: "Matching failed" }, {
            status: resp.status,
          });
        }
      ...
      case "scan": {
        ...
        const data = await resp.json().catch(() => ({}));
        if (!resp.ok) {
          return NextResponse.json(data || { error: "Scan failed" }, {
            status: resp.status,
          });
        }
