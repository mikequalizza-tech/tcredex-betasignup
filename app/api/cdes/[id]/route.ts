 ...
 */
async function resolveCdeId(
  supabase: ReturnType<typeof getSupabaseAdmin>,
  idOrOrgId: string,
): Promise<{
  id: string;
  orgId: string;
  source: "cdes" | "cdes_merged";
} | null> {
  ...
  if (byId)
    return {
      ...
      orgId: (byId as { organization_id: string }).organization_id,
      source: "cdes",
    };
  ...
  if (byOrgId)
    return {
      id: (byOrgId as { id: string }).id,
      orgId: (byOrgId as { organization_id: string }).organization_id,
      source: "cdes",
    };
  ...
  if (merged)
    return {
      id: (merged as { id: string }).id,
      orgId: (merged as { organization_id: string }).organization_id,
      source: "cdes_merged",
    };
...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const isOwner =
      user.organizationType === "cde" && resolved.orgId === user.organizationId;
...
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (resolved.source === "cdes_merged") {
      return NextResponse.json(
        { error: "CDE profile is read-only (imported from CDFI Fund)" },
        { status: 403 },
      );
    }
    ...
    if (allocations && Array.isArray(allocations)) {
      ...
      if (allocations.length > 0) {
        const allocationsToInsert = allocations.map(
        ...
        await supabase
          ...
          .insert(allocationsToInsert as never);
      }

      const totalAllocation = allocations.reduce(
        (sum: number, a: Record<string, number>) =>
          sum + (a.awarded_amount || 0),
        0,
      );
      const remainingAllocation = allocations.reduce(
        (sum: number, a: Record<string, number>) =>
          sum + (a.available_on_platform || 0),
        0,
      );
