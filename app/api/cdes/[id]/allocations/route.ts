 ...
 */
async function resolveCde(supabase: ReturnType<typeof getSupabaseAdmin>, idOrOrgId: string) {
  ...
  if (byId) return { ...(byId as { id: string; organization_id: string }), source: 'cdes' as const };
  ...
  if (byOrgId) return { ...(byOrgId as { id: string; organization_id: string }), source: 'cdes' as const };
  ...
  if (merged) return { ...(merged as { id: string; organization_id: string }), source: 'cdes_merged' as const };
...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const isOwner = user.organizationType === 'cde' && cde.organization_id === user.organizationId;
    if (!isAdmin && !isOwner) {
    ...
    const { data: mergedRows, error: mergedError } = await supabase
      .from('cdes_merged')
      .select('id, year, total_allocation, amount_remaining, amount_finalized, non_metro_commitment, allocation_type')
    ...
    const allocations = (mergedRows || []).map((row: Record<string, unknown>) => ({
      id: row.id,
      cde_id: cde.id,
