...
function enrichFromQEI(cde: Record<string, unknown>): Record<string, unknown> {
  ...
  if (
    ...
    (cde.primary_states as string[]).length === 0
  ) {
  ...
  if (
    ...
    (cde.target_sectors as string[]).length === 0
  ) {
  ...
  if (
    !cde.special_focus ||
    !Array.isArray(cde.special_focus) ||
    (cde.special_focus as string[]).length === 0
  ) {
    const focus: string[] = [];
    if (
      innovativeActivities.includes("minority") ||
      innovativeActivities.includes("african") ||
      innovativeActivities.includes("hispanic") ||
      innovativeActivities.includes("latino")
    ) {
    ...
    if (
      innovativeActivities.includes("indian country") ||
      innovativeActivities.includes("tribal") ||
      innovativeActivities.includes("native")
    ) {
...
function aggregateCDEs(
  ...
  for (const [orgId, yearRows] of grouped) {
    ...
    aggregated.push({
      ...
      non_metro_commitment:
        yearRows.length > 0 ? nonMetroSum / yearRows.length : 0,
      // Year info
  ...
  aggregated.sort(
    (a, b) =>
      (Number(b.remaining_allocation) || 0) -
      (Number(a.remaining_allocation) || 0),
  );
  return aggregated;
}
  ...
  try {
    ...
    if (minAllocation) {
      filtered = filtered.filter(
        (c) => (Number(c.remaining_allocation) || 0) >= parseInt(minAllocation),
      );
    }
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        amount_remaining:
          body.remaining_allocation || body.total_allocation || 0,
