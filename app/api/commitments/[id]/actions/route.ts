  ...
  try {
    ...
    switch (action) {
      case "issue":
        if (
          ...
          return NextResponse.json(
            {
              success: false,
              error: "Only the issuing investor can issue this commitment",
            },
            { status: 403 },
          );
        }
      ...
      case "send":
        if (
          ...
          return NextResponse.json(
            {
              success: false,
              error: "Only the issuing investor can send this commitment",
            },
            { status: 403 },
          );
        }
      ...
      case "sponsor_accept":
        if (
          user.organizationType !== "admin" &&
          !(await canUserActAsSponsor(user.organizationId, existing.sponsor_id))
        ) {
          return NextResponse.json(
            {
              success: false,
              error: "Only the sponsor can accept this commitment",
            },
            { status: 403 },
          );
        }
        commitment = await service.sponsorAccept(
          id,
          existing.sponsor_id,
          notes,
        );
      ...
      case "cde_accept":
        if (!existing.cde_id) {
          return NextResponse.json(
            {
              success: false,
              error: "This commitment does not require CDE acceptance",
            },
            { status: 400 },
          );
        }
        if (
          user.organizationType !== "admin" &&
          !(await canUserActAsCde(user.organizationId, existing.cde_id))
        ) {
          return NextResponse.json(
            {
              success: false,
              error: "Only the CDE can accept this commitment",
            },
            { status: 403 },
          );
        }
      ...
      case "reject":
        ...
        if (
          user.organizationType !== "admin" &&
          !(await canUserParticipateInCommitment(user.organizationId, existing))
        ) {
          return NextResponse.json(
            {
              success: false,
              error: "Only deal participants can reject this commitment",
            },
            { status: 403 },
          );
        }
      ...
      case "withdraw":
        ...
        if (
          ...
          return NextResponse.json(
            {
              success: false,
              error: "Only the issuing investor can withdraw this commitment",
            },
            { status: 403 },
          );
        }
...
async function canUserActAsInvestor(
  ...
  return (
    !!investor &&
    (investor as { organization_id: string }).organization_id ===
      userOrganizationId
  );
}

async function canUserActAsSponsor(
  userOrganizationId: string,
  sponsorId: string,
): Promise<boolean> {
  const supabase = getSupabaseAdmin();
...
async function canUserActAsCde(
  userOrganizationId: string,
  cdeId: string,
): Promise<boolean> {
  const supabase = getSupabaseAdmin();
  ...
  if (
    cde &&
    (cde as { organization_id: string }).organization_id === userOrganizationId
  ) {
    return true;
  }
...
async function canUserParticipateInCommitment(
  ...
  if (
    commitment.cde_id &&
    (await canUserActAsCde(userOrganizationId, commitment.cde_id))
  ) {
    return true;
  }
