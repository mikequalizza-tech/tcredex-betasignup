...
export async function GET(
  ...
  try {
    ...
    await verifyDealAccess(request, user, (data as { deal_id: string }).deal_id, 'view');

    // Get category metadata
    const categoryMeta = DD_CATEGORIES.find((c) => c.category === data.category);
...
export async function PATCH(
  ...
  try {
    ...
    await verifyDealAccess(request, user, (existingDocument as { deal_id: string }).deal_id, 'edit');
    ...
    if (body.status) {
      ...
      if (status === 'approved' && body.documentId) {
        ...
        if (currentDoc) {
          const categoryMeta = DD_CATEGORIES.find((c) => c.category === currentDoc.category);
          if (categoryMeta?.expirationDays) {
            const expirationDate = new Date();
            expirationDate.setDate(expirationDate.getDate() + categoryMeta.expirationDays);
