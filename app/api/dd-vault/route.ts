...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`[DD Vault GET] dealId=${dealId}, found ${documents?.length || 0} documents`);
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    console.log(`[DD Vault DELETE] Deleted ${existingCount} documents for dealId=${dealId}`);
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (existing && existing.length > 0) {
      return NextResponse.json(
        { error: 'DD vault already exists for this deal', existingCount: existing.length },
        { status: 409 }
      );
    }
