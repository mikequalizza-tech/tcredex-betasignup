...
export default function NewDealPage() {
      ...
      <div className="min-h-screen bg-gray-950 flex items-center justify-center">
        <div className="text-center max-w-md p-8 bg-gray-900 rounded-xl border border-gray-800">
          ...
          <p className="text-gray-400 mb-6">
            ...
            {orgType === "investor" &&
              " As an Investor, you can browse and invest in deals from the Marketplace."}
          </p>
  ...
  const handleSave = async (
    ...
    if (!sponsorId) {
      const error = new Error(
        "No organization ID available. Please try logging out and back in.",
      );
      console.error(
        "No sponsorId available - user may not be properly authenticated",
      );
  ...
  const handleSubmit = async (
    ...
    if (!sponsorId) {
      ...
      alert(
        "Unable to submit: No organization ID available. Please try logging out and back in.",
      );
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        const error = await response.json();
        throw new Error(
          error.message || error.error || "Failed to submit deal",
        );
      }
