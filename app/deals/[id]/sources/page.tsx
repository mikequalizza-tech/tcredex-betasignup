...
export default function SourcesPage() {
  ...
  const [capitalSummary, setCapitalSummary] = useState<CapitalStackSummary | null>(null);
  const [relationships, setRelationships] = useState<PipelineRelationship[]>([]);
  const [pipelineSummary, setPipelineSummary] = useState<PipelineSummary | null>(null);
  const [activeTab, setActiveTab] = useState<'pipeline' | 'sources'>('pipeline');
  ...
  const progressPercent = capitalSummary && allocationNeeded > 0
    ? Math.min(100, Math.round((capitalSummary.totalCommitted / allocationNeeded) * 100))
    : 0;
      ...
      <div className="border-b border-gray-800">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 py-6">
          <div className="flex items-center gap-4">
            ...
            <button
              onClick={() => router.push(`/dashboard/automatch?dealId=${dealId}`)}
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 py-8">
        ...
        <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl border border-gray-700 p-6 mb-8">
          <div className="flex items-start justify-between mb-6">
            <div>
              <h2 className="text-lg font-semibold text-white mb-1">Path to Closing</h2>
              <p className="text-gray-400 text-sm">Track your progress from gap to close</p>
            </div>
          ...
          <div className="mb-6">
            <div className="flex justify-between text-sm mb-2">
              ...
              <span className={`font-bold ${
                ...
                progressPercent >= 75 ? 'text-emerald-400' :
                progressPercent >= 50 ? 'text-amber-400' :
                'text-gray-400'
              }`}>
                {progressPercent}%
              </span>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-gray-800/50 rounded-xl p-4 border border-gray-700">
              ...
              <p className="text-2xl font-bold text-white">{formatCurrency(allocationNeeded)}</p>
            </div>
          ...
          <div className="space-y-6">
                ...
                {STAGE_PROGRESSION.map((stage) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className={`text-sm font-bold ${count > 0 ? 'text-white' : 'text-gray-600'}`}>
                        {count}
                      </div>
                    </div>
                  );
            ...
            {activeRelationships.length > 0 ? (
                  ...
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                          rel.targetType === 'cde' ? 'bg-emerald-900/50' : 'bg-blue-900/50'
                        }`}>
                        ...
                        <div>
                          <p className="font-medium text-white">{rel.targetName}</p>
                          <p className="text-sm text-gray-400 capitalize">{rel.targetType}</p>
                        </div>
                      </div>

                      <div className="text-right">
                        <span className={`inline-block px-3 py-1 rounded-lg text-sm font-medium ${rel.stageBgColor} ${rel.stageColor}`}>
                          {rel.stageLabel}
                        </span>
                        {rel.matchScore && (
                          <p className="text-xs text-gray-500 mt-1">{rel.matchScore}% match</p>
                        )}
                      </div>
                    </div>
                      ...
                      <div className="flex gap-4 text-sm">
                        {rel.requestedAmount && (
                          <div>
                            ...
                            <span className="text-white font-medium">{formatCurrency(rel.requestedAmount)}</span>
                          </div>
                        )}
                        {rel.committedAmount && (
                          <div>
                            <span className="text-gray-500">Committed: </span>
                            <span className="text-emerald-400 font-medium">{formatCurrency(rel.committedAmount)}</span>
                          </div>
                        )}
                        {rel.lastActivity && (
                    ...
                    {rel.nextAction && (
                      <div className="mt-3 pt-3 border-t border-gray-800 flex items-center gap-2 text-sm">
                        ...
                        {rel.nextActionDue && (
                          <span className="text-gray-500">by {formatDate(rel.nextActionDue)}</span>
                        )}
                      </div>
                    )}
                  </div>
              ...
              <div className="bg-gray-900 rounded-xl border border-gray-800 p-8 text-center">
                ...
                <h3 className="text-xl font-semibold text-white mb-2">No Active Sources Yet</h3>
                <p className="text-gray-400 mb-6">
                  Run AutoMatch to find CDEs and Investors that align with your project.
                </p>
                <button
                  onClick={() => router.push(`/dashboard/automatch?dealId=${dealId}`)}
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-gray-400">{rel.targetName}</span>
                        </div>
                        <span className={`text-sm ${rel.stageColor}`}>{rel.stageLabel}</span>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        {activeTab === 'sources' && (
          <div className="space-y-4">
            {capitalSources.length > 0 ? (
              capitalSources.map((source) => (
                <div
                  ...
                  className={`bg-gray-900 rounded-xl border p-5 ${
                    source.type === 'commitment' ? 'border-emerald-500/30' :
                    source.type === 'loi' ? 'border-amber-500/30' :
                    'border-gray-800'
                  }`}
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex items-center gap-3">
                      <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${
                        source.type === 'commitment' ? 'bg-emerald-900/50' :
                        source.type === 'loi' ? 'bg-amber-900/50' :
                        'bg-gray-800'
                      }`}>
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-400 capitalize">
                          {source.type === 'loi' ? 'Letter of Intent' :
                           source.type === 'commitment' ? 'Commitment Letter' :
                           source.sourceType}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="text-right">
                      <p className={`text-2xl font-bold ${
                        source.type === 'commitment' ? 'text-emerald-400' :
                        source.type === 'loi' ? 'text-amber-400' :
                        'text-white'
                      }`}>
                        {formatCurrency(source.amount)}
                      </p>
                      <span className={`inline-block mt-1 px-2 py-0.5 rounded text-xs font-medium ${
                        source.status === 'all_accepted' || source.status === 'sponsor_accepted'
                          ? 'bg-green-900/50 text-green-400'
                          : source.status === 'expired' || source.status === 'rejected'
                  ...
                  <div className="mt-4 pt-4 border-t border-gray-800 flex flex-wrap gap-4 text-sm">
                    {source.issuedAt && (
                      <div>
                        ...
                        <span className="text-gray-300">{formatDate(source.issuedAt)}</span>
                      </div>
                    )}
                    {source.expiresAt && (
                      <div>
                        ...
                        <span className={`${
                          new Date(source.expiresAt) < new Date() ? 'text-red-400' : 'text-gray-300'
                        }`}>
                    ...
                    {source.contactName && (
                      <div>
                        ...
                        <span className="text-gray-300">{source.contactName}</span>
                      </div>
                    )}
                    {source.creditType && (
                      <div>
                        <span className="text-gray-500">Credit: </span>
                        <span className="text-indigo-400">{source.creditType}</span>
                      </div>
                    )}
                  </div>

                  {/* Action Buttons */}
                  {(source.status === 'issued' || source.status === 'pending_sponsor') && (
              ...
              <div className="bg-gray-900 rounded-xl border border-gray-800 p-8 text-center">
                <div className="text-5xl mb-4">📝</div>
                <h3 className="text-xl font-semibold text-white mb-2">No LOIs or Commitments Yet</h3>
                <p className="text-gray-400">
                  When CDEs or Investors issue Letters of Intent or Commitment Letters, they'll appear here.
                </p>
              </div>
            )}
          </div>
        ...
        {capitalSummary?.readyForClosing && (
          <div className="mt-8 bg-gradient-to-r from-green-900/30 to-emerald-900/30 border border-green-500/30 rounded-xl p-6 text-center">
            ...
            <h3 className="text-xl font-bold text-white mb-2">Your Deal is Fully Funded!</h3>
