...
/** Organization form data — union of all role-specific fields */
interface OrgFormData {
  ...
  programs?: string[];
  [key: string]: unknown; // allow additional dynamic fields from API
}

/** Raw CDE row from API */
interface RawCDE {
  id: string;
  organization_id?: string;
  primary_states?: string[];
  target_sectors?: string[];
  min_deal_size?: string | number;
  max_deal_size?: string | number;
  small_deal_fund?: boolean;
  require_severely_distressed?: boolean;
  require_qct?: boolean;
  preferred_project_types?: string[];
...
/** Raw Investor row from API */
interface RawInvestor {
  id: string;
  min_investment?: string | number;
  max_investment?: string | number;
  target_states?: string[];
  preferred_sectors?: string[];
  target_credit_types?: string[];
}
...
function OrganizationInfoContent() {
  ...
  useEffect(() => {
    async function loadOrganizationData() {
      ...
      try {
        ...
        const result = await fetchApi<{ organization: OrgFormData }>(
          `/api/organization?id=${encodeURIComponent(organizationId)}`,
        );
        if (result.success && result.data?.organization) {
          setFormData(result.data.organization);
        } else {
          // Organization doesn't exist yet - pre-populate with available data from user context
          console.log("Organization not found, initializing with defaults");
          setFormData({
            name: orgName || "",
        ...
        if (orgType === "cde") {
          try {
            ...
            const cdeResult = await fetchApi<{ cdes?: RawCDE[] }>(`/api/cdes`);
            if (
              cdeResult.success &&
              cdeResult.data?.cdes &&
              Array.isArray(cdeResult.data.cdes) &&
              cdeResult.data.cdes.length > 0
            ) {
              // Find CDE matching this organization
              const cdeData =
                cdeResult.data.cdes.find(
                  (cde) => cde.organization_id === organizationId,
                ) || cdeResult.data.cdes[0];
              if (cdeData) {
                setCdeId(cdeData.id);
                setFormData((prev) => ({
                  ...prev,
                  ...
                  min_deal_size: cdeData.min_deal_size
                    ? parseFloat(cdeData.min_deal_size.toString())
                    : 0,
                  max_deal_size: cdeData.max_deal_size
                    ? parseFloat(cdeData.max_deal_size.toString())
                    : 0,
                  small_deal_fund: cdeData.small_deal_fund || false,
                  require_severely_distressed:
                    cdeData.require_severely_distressed || false,
                  require_qct: cdeData.require_qct || false,
                  preferred_project_types:
                    cdeData.preferred_project_types || [],
                  service_area_type: cdeData.service_area_type || "national",
                  rural_focus: cdeData.rural_focus || false,
                  urban_focus:
                    cdeData.urban_focus !== undefined
                      ? cdeData.urban_focus
                      : true,
                }));
              }
        ...
        if (orgType === "investor") {
          try {
            const investorResult = await fetchApi<{
              investors?: RawInvestor[];
            }>(`/api/investors`);
            if (
              investorResult.success &&
              investorResult.data?.investors &&
              Array.isArray(investorResult.data.investors) &&
              investorResult.data.investors.length > 0
            ) {
              const investorData = investorResult.data.investors[0];
              setFormData((prev) => ({
                ...prev,
                // AUTOMATCH fields from investors table
                min_investment: investorData.min_investment
                  ? parseFloat(investorData.min_investment.toString())
                  : 0,
                max_investment: investorData.max_investment
                  ? parseFloat(investorData.max_investment.toString())
                  : 0,
  ...
  }, [organizationId, orgType, orgName]);
      ...
      if (orgType === "cde" && cdeId) {
        try {
          const cdeUpdateData: Record<string, unknown> = {
            ...
            preferred_project_types: Array.isArray(preferred_project_types)
              ? preferred_project_types
              : [],
        ...
        try {
          const investorUpdateData: Record<string, unknown> = {
            ...
            preferred_sectors: Array.isArray(preferred_sectors)
              ? preferred_sectors
              : [],
            target_credit_types: Array.isArray(target_credit_types)
              ? target_credit_types
              : [],
          };

          const investorResult = await apiPut<{ success: boolean }>(
            "/api/investors",
            investorUpdateData,
          );
          if (!investorResult.success) {
            console.warn(
              "Failed to save investor AUTOMATCH data:",
              investorResult,
            );
            // Don't fail the whole save, just warn
          }
        ...
        <div className="max-w-4xl mx-auto">
          <div className="mb-6">
            ...
            <p className="text-gray-400 mt-1">
              Complete your organization profile to improve AutoMatch accuracy
              and help CDEs and investors find you.
            </p>
          </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-sm text-gray-400">
                  {orgType === "sponsor" &&
                    "Complete your sponsor profile to help CDEs and investors understand your organization."}
                  {orgType === "cde" &&
                    "Complete your CDE profile to help AutoMatch connect you with the right projects."}
                  {orgType === "investor" &&
                    "Complete your investor profile to help AutoMatch connect you with the right opportunities."}
                  {!orgType &&
                    "Complete your organization profile to improve AutoMatch accuracy."}
                </p>
              </div>
            ...
            {orgType === "cde" && (
              <div className="pt-6 border-t border-gray-800 space-y-6">
                <div>
                  ...
                  <p className="text-sm text-gray-400">
                    Configure your matching preferences to help AutoMatch
                    connect you with the right projects.
                  </p>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-300 mb-2">
                      Primary States (comma-separated state codes, e.g., CA, NY,
                      TX)
                    </label>
                    <input
                      type="text"
                      value={
                        Array.isArray(formData.primary_states)
                          ? formData.primary_states.join(", ")
                          : formData.primary_states || ""
                      }
                      onChange={(e) => {
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      Enter 2-letter state codes separated by commas
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input
                        type="checkbox"
                        checked={
                          formData.urban_focus !== undefined
                            ? formData.urban_focus
                            : true
                        }
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            urban_focus: e.target.checked,
                          })
                        }
                        className="w-4 h-4 text-indigo-600 bg-gray-800 border-gray-700 rounded focus:ring-indigo-500"
                      />
                ...
                <div className="pt-4 border-t border-gray-800 space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      value={
                        Array.isArray(formData.target_sectors)
                          ? formData.target_sectors.join(", ")
                    ...
                    />
                    <p className="text-xs text-gray-500 mt-1">
                      Enter sectors separated by commas (e.g., Healthcare,
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            min_deal_size: parseFloat(e.target.value) || 0,
                          })
                        }
                        className="w-full px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg text-gray-100 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                        placeholder="1000000"
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            max_deal_size: parseFloat(e.target.value) || 0,
                          })
                        }
                        className="w-full px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg text-gray-100 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                  ...
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      ...
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                          small_deal_fund: e.target.checked,
                        })
                      }
                      className="w-4 h-4 text-indigo-600 bg-gray-800 border-gray-700 rounded focus:ring-indigo-500"
                    />
                    <span className="text-sm text-gray-300">
                      Small Deal Fund (accepts deals under $5M)
                    </span>
                  </label>
                </div>
                ...
                <div className="pt-4 border-t border-gray-800 space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      value={
                        Array.isArray(formData.preferred_project_types)
                          ? formData.preferred_project_types.join(", ")
                          : formData.preferred_project_types || ""
                      }
                      onChange={(e) => {
                    ...
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input
                        ...
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            require_severely_distressed: e.target.checked,
                          })
                        }
                        className="w-4 h-4 text-indigo-600 bg-gray-800 border-gray-700 rounded focus:ring-indigo-500"
                      />
                      <span className="text-sm text-gray-300">
                        Require Severely Distressed Tracts
                      </span>
                    </label>
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input
                        ...
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            require_qct: e.target.checked,
                          })
                        }
                        className="w-4 h-4 text-indigo-600 bg-gray-800 border-gray-700 rounded focus:ring-indigo-500"
                      />
                      <span className="text-sm text-gray-300">
                        Require QCT (Qualified Census Tract)
                      </span>
                    </label>
                  </div>
                </div>
            ...
            {orgType === "investor" && (
              <div className="pt-6 border-t border-gray-800 space-y-6">
                <div>
                  ...
                  <p className="text-sm text-gray-400">
                    Configure your investment criteria to help AutoMatch connect
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            min_investment: parseFloat(e.target.value) || 0,
                          })
                        }
                        className="w-full px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg text-gray-100 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                        placeholder="1000000"
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            max_investment: parseFloat(e.target.value) || 0,
                          })
                        }
                        className="w-full px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg text-gray-100 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                ...
                <div className="pt-4 border-t border-gray-800 space-y-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-300 mb-2">
                      Preferred States (comma-separated state codes, e.g., CA,
                      NY, TX)
                    </label>
                    <input
                      type="text"
                      value={
                        Array.isArray(formData.target_states)
                          ? formData.target_states.join(", ")
                          : formData.target_states ||
                            formData.preferred_states ||
                            ""
                      }
                      onChange={(e) => {
                        const states = e.target.value
                ...
                <div className="pt-4 border-t border-gray-800 space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      value={
                        Array.isArray(formData.preferred_sectors)
                          ? formData.preferred_sectors.join(", ")
                ...
                <div className="pt-4 border-t border-gray-800 space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      value={
                        Array.isArray(formData.target_credit_types)
                          ? formData.target_credit_types.join(", ")
                          : formData.target_credit_types ||
                            formData.programs ||
                            ""
                      }
                      onChange={(e) => {
                    ...
                    />
                    <p className="text-xs text-gray-500 mt-1">
                      Enter program codes: NMTC, LIHTC, HTC, OZ
                    </p>
                  </div>
                </div>
              </div>
