  ...
  useEffect(() => {
    async function loadSupabaseDeals() {
      ...
      try {
        ...
        const fetched = await fetchDealsByOrganization(organizationId || '', userEmail, effectiveRole);
        ...
        const mapped: PipelineDeal[] = fetched.map(d => {
          ...
          return {
            ...
            readinessScore: (d as unknown as { readinessScore?: number; readiness_score?: number }).readinessScore || (d as unknown as { readiness_score?: number }).readiness_score,
            matchScore: 0,
        ...
        if ((effectiveRole === 'cde' || effectiveRole === 'investor') && organizationId) {
          try {
        ...
        const filtered = effectiveRole === 'sponsor'
  ...
  useEffect(() => {
    ...
    const loadDrafts = async () => {
      ...
      try {
        ...
        const result = await fetchApi<{ drafts?: RawDraft[]; draft?: RawDraft }>(`/api/drafts?orgId=${encodeURIComponent(organizationId)}`);
        ...
        if (draftsList.length > 0) {
          const mappedDrafts: PipelineDeal[] = draftsList.map((draft: RawDraft) => {
            // Get draft data - handle both draft_data and intake_data
            const draftData = (draft.draft_data || draft.intake_data || {}) as Record<string, unknown>;
            
            // Extract program type from draft data
            const programs = (draftData.programs || draft.programs) as string[] | undefined;
            const draftProgram = Array.isArray(programs) && programs.length > 0 ? programs[0] : null;

            return {
              id: draft.id,
              projectName: draft.project_name || (draftData.projectName as string) || 'Untitled Draft',
              sponsorName: (draftData.sponsorName as string) || draft.sponsor_name || 'Not specified',
              city: (draftData.city as string) || draft.city || '',
              state: (draftData.state as string) || draft.state || '',
              programType: (draftProgram || 'DRAFT') as PipelineDeal['programType'],
              allocationRequest: Number(draftData.totalProjectCost || draft.total_project_cost || 0),
              stage: 'draft' as const,
              matchScore: draft.readiness_score || 0,
              tractType: [],
              daysInStage: Math.floor((Date.now() - new Date(draft.updated_at || draft.created_at || Date.now()).getTime()) / (1000 * 60 * 60 * 24)),
          ...
          <table className="w-full">
            <thead className="bg-gray-800/50">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase">C-Score</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase" title="CDE matches - run AutoMatch on deal page">Matches</th>
            ...
            <tbody className="divide-y divide-gray-800">
                ...
                >
                  <td className="px-4 py-3">
                    <div>
                      ...
                      {(draft.city || draft.state) && (
                        <p className="text-sm text-gray-500 mt-0.5">{draft.city}{draft.city && draft.state ? ', ' : ''}{draft.state}</p>
                      )}
                    </div>
                  </td>
                  <td className="px-4 py-3">
                    {draft.programType && draft.programType !== 'DRAFT' ? (
                      <span className={`px-2 py-1 rounded text-xs font-medium ${
                        ...
                        } as Record<string, string>)[draft.programType] || 'bg-gray-700 text-gray-300'
                      }`}>
                        {draft.programType}
                      </span>
                    ) : (
                  ...
                  <td className="px-4 py-3">
                    {draft.allocationRequest > 0 ? (
                      <span className="font-medium text-gray-100">{formatCurrency(draft.allocationRequest)}</span>
                    ) : (
                      <span className="text-gray-500">—</span>
                    )}
                  </td>
                  <td className="px-4 py-3">
                    {draft.readinessScore ? (
                      <span className={`text-sm font-medium ${getScoreColor(draft.readinessScore)}`}>{draft.readinessScore}%</span>
                    ) : (
              ...
              {supabaseDeals
                ...
                .filter(deal => !(effectiveRole === 'sponsor' && deal.stage === 'draft'))
                .filter(deal => {
                  ...
                  if (effectiveRole === 'sponsor' && drafts.length > 0) {
                    const draftNames = drafts.map(d => d.projectName?.toLowerCase().trim());
                    return !draftNames.includes(deal.projectName?.toLowerCase().trim());
                  }
                  return true;
                })
                ...
                return (
                  <tr key={`deal-${deal.id}`} className="hover:bg-gray-800/50 cursor-pointer" onClick={() => handleDealClick(deal)}>
                    ...
                    <td className="px-4 py-3">
                      <span className={`px-2 py-1 rounded text-xs font-medium ${programColors[deal.programType] || 'bg-gray-700 text-gray-300'}`}>
                        {deal.programType}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      {deal.readinessScore ? (
                        <span className={`text-sm font-medium ${getScoreColor(deal.readinessScore)}`}>{deal.readinessScore}%</span>
                      ) : (
                        <span className="text-gray-500 text-sm">—</span>
                      )}
                    </td>
                    ...
                    <td className="px-4 py-3 text-gray-400 text-sm">{deal.daysInStage}d</td>
                    <td className="px-4 py-3">
          ...
          <div className="relative bg-gray-900 rounded-xl p-6 w-full max-w-lg mx-4 border border-gray-800">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs text-gray-500 uppercase">Program</label>
                  <p className="text-lg font-semibold text-emerald-400">{selectedDeal.programType}</p>
                </div>
                <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs text-gray-500 uppercase">C-Score</label>
                  <p className={`text-lg font-semibold ${getScoreColor(selectedDeal.readinessScore || 0)}`}>
                    {selectedDeal.readinessScore ? `${selectedDeal.readinessScore}%` : '—'}
                  </p>
                </div>
                <div>
                  <label className="text-xs text-gray-500 uppercase">CDE Matches</label>
                  <p className="text-lg font-semibold text-gray-400">Run AutoMatch</p>
                </div>
              </div>
