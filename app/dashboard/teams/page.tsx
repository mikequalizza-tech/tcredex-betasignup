...
export default function TeamPage() {
  const { userName, userEmail, orgName, orgType, organizationId, canEditOrganization, isOrgAdmin } = useCurrentUser();
  const [members, setMembers] = useState<TeamMember[]>([]);
  const [loading, setLoading] = useState(true);
  const [showInviteModal, setShowInviteModal] = useState(false);

  // Map API roles to frontend roles
  const mapApiRoleToFrontend = (apiRole: string): MemberRole => {
  ...
  const fetchTeamMembers = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const transformedMembers: TeamMember[] = (data.members || []).map((m: {
          id: string;
          name: string;
          email: string;
          role: string;
          ...
          role: mapApiRoleToFrontend(m.role),
          avatar: m.avatar_url,
          lastActive: m.last_login_at ? new Date(m.last_login_at).toLocaleDateString() : 'Never',
          status: m.is_active ? 'active' : 'inactive',
        }));

        // If no members returned, show current user
  ...
  const [inviteSuccess, setInviteSuccess] = useState<{ email: string; name: string; emailSent: boolean; error?: string } | null>(null);
  ...
  // Sponsor-specific fields (ownership certifications)
  const [orgEntityType, setOrgEntityType] = useState('');
  const [lowIncomeOwned, setLowIncomeOwned] = useState(false);
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.organization) {
            ...
            setOrgPhone(data.organization.primary_contact_phone || '');
            ...
            // Logo
            setOrgLogoUrl(data.organization.logo_url || '');
            // Sponsor-specific fields
            setOrgEntityType(data.organization.organization_type || '');
            setLowIncomeOwned(data.organization.low_income_owned || false);
            setWomanOwned(data.organization.woman_owned || false);
            setMinorityOwned(data.organization.minority_owned || false);
            setVeteranOwned(data.organization.veteran_owned || false);
          }
        }
      } catch (error) {
    ...
    try {
      const response = await fetch('/api/organization', {
        ...
        body: JSON.stringify({
          ...
          // Sponsor-specific fields
          organization_type: orgEntityType,
          low_income_owned: lowIncomeOwned,
      ...
      if (response.ok) {
        ...
        // Reload to refresh user context (useCurrentUser reads organization_name on mount)
  ...
  const handleLogoFile = async (file: File) => {
    ...
    try {
      ...
      const response = await fetch('/api/upload/org-logo', { method: 'POST', body: formData });
      if (!response.ok) {
        const data = await response.json();
      ...
      if (organizationId) {
        await fetch('/api/organization', {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ id: organizationId, logo_url: url }),
        });
      }
    } catch (err) {
  ...
  const handleLogoRemove = async () => {
    ...
    if (organizationId) {
      await fetch('/api/organization', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id: organizationId, logo_url: null }),
      });
    }
  };

  // Access control - only ORG_ADMIN can edit organization settings
  const canEditOrg = typeof canEditOrganization === 'function' ? canEditOrganization() : isOrgAdmin;
  ...
  const handleSendInvite = async () => {
    ...
    try {
      ...
      const apiRoleMap: Record<MemberRole, string> = {
        owner: 'ORG_ADMIN',
        admin: 'ORG_ADMIN',
        member: 'MEMBER',
        viewer: 'VIEWER',
      ...
      const response = await fetch('/api/team', {
        ...
        body: JSON.stringify({
          ...
          role: apiRoleMap[inviteRole] || 'MEMBER',
      ...
      if (response.ok) {
        ...
        const newMember: TeamMember = {
          ...
          name: data.user?.name || inviteEmail.split('@')[0].replace(/[._]/g, ' ').replace(/\b\w/g, c => c.toUpperCase()),
          email: inviteEmail,
        ...
        setInviteSuccess({
          ...
          error: data.emailSent ? undefined : (data.message || 'Check Resend domain verification and API key.'),
        });
        setInviteEmail('');
    ...
    <div className="p-8">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-400 mt-1">Manage your organization profile and team members</p>
        </div>
        {isOrgAdmin && (
          <button
      ...
      <div className="bg-gray-900 rounded-xl border border-gray-800 p-6 mb-8">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            {orgLogoUrl ? (
              <div className="w-14 h-14 rounded-xl bg-gray-800 border border-gray-700 overflow-hidden relative flex-shrink-0">
                <Image src={orgLogoUrl} alt={organizationName} fill className="object-contain p-1" />
              </div>
            ) : (
              <div className="w-14 h-14 rounded-xl bg-indigo-600/20 border border-indigo-600/30 flex items-center justify-center flex-shrink-0">
      ...
      <div className="bg-gray-900 rounded-xl border border-gray-800 mb-8 overflow-hidden">
        ...
        {showAboutSection && (
          <div className="p-6 pt-2 border-t border-gray-800">
              ...
              <div className="flex items-start gap-4">
                {orgLogoUrl ? (
                  <div className="relative group">
                    <div className="w-24 h-24 rounded-xl bg-gray-800 border border-gray-700 overflow-hidden relative">
                      <Image src={orgLogoUrl} alt="Organization logo" fill className="object-contain p-1" />
                    </div>
                    {canEditOrg && (
                  ...
                  <div
                    ...
                    onDrop={(e) => { e.preventDefault(); setLogoDragActive(false); if (e.dataTransfer.files?.[0]) handleLogoFile(e.dataTransfer.files[0]); }}
                    onClick={() => logoInputRef.current?.click()}
                  ...
                  <div className="w-24 h-24 rounded-xl bg-gray-800/50 border border-gray-700/50 flex items-center justify-center">
                ...
                <div className="text-sm text-gray-400 pt-1">
                  <p>Your logo appears on deal exports, CDE profile pages, and Project Profiles.</p>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => canEditOrg && setOrganizationName(e.target.value)}
                    placeholder="Your organization name"
                    readOnly={!canEditOrg}
                    className={`w-full px-3 py-2 border rounded-lg text-gray-100 placeholder-gray-500 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 ${
                      canEditOrg
                  ...
                  {!canEditOrg && (
                    <p className="text-xs text-amber-500 mt-1 flex items-center gap-1">
                      ...
                      Only Organization Admins can change the organization name
                    </p>
                  )}
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) => canEditOrg && setOrgDescription(e.target.value)}
                    placeholder="Brief description of your organization, mission, and experience..."
                    rows={4}
                    readOnly={!canEditOrg}
                    className={`w-full px-3 py-2 border rounded-lg text-gray-100 placeholder-gray-500 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 ${
                      canEditOrg ? 'bg-gray-800 border-gray-700' : 'bg-gray-800/50 border-gray-700/50 cursor-not-allowed'
                    }`}
                  />
                ...
                <div>
                  ...
                  <input
                    type="url"
                    value={orgWebsite}
                    onChange={(e) => canEditOrg && setOrgWebsite(e.target.value)}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => canEditOrg && setOrgYearFounded(e.target.value)}
                    placeholder="2010"
                    maxLength={4}
                    readOnly={!canEditOrg}
                    className={`w-full px-3 py-2 border rounded-lg text-gray-100 placeholder-gray-500 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 ${
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => canEditOrg && setOrgPrimaryContact(e.target.value)}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => canEditOrg && setOrgPrimaryEmail(e.target.value)}
                ...
                <div>
                  ...
                  <input
                    type="tel"
                    value={orgPhone}
                    onChange={(e) => canEditOrg && setOrgPhone(e.target.value)}
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={orgAddress}
                    onChange={(e) => canEditOrg && setOrgAddress(e.target.value)}
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div className="col-span-1">
                    ...
                    <input
                      type="text"
                      value={orgCity}
                      onChange={(e) => canEditOrg && setOrgCity(e.target.value)}
                      placeholder="City"
                      readOnly={!canEditOrg}
                      className={`w-full px-3 py-2 border rounded-lg text-gray-100 placeholder-gray-500 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 ${
                        canEditOrg ? 'bg-gray-800 border-gray-700' : 'bg-gray-800/50 border-gray-700/50 cursor-not-allowed'
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      value={orgState}
                      onChange={(e) => canEditOrg && setOrgState(e.target.value)}
                      placeholder="ST"
                      maxLength={2}
                      readOnly={!canEditOrg}
                      className={`w-full px-3 py-2 border rounded-lg text-gray-100 placeholder-gray-500 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 ${
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      value={orgZip}
                      onChange={(e) => canEditOrg && setOrgZip(e.target.value)}
                      placeholder="12345"
                      maxLength={10}
                      readOnly={!canEditOrg}
                      className={`w-full px-3 py-2 border rounded-lg text-gray-100 placeholder-gray-500 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 ${
            ...
            {/* Sponsor-Specific: Entity Type & Ownership Certifications */}
            {orgType === 'sponsor' && (
              <div className="pt-6 border-t border-gray-800">
                ...
                <p className="text-sm text-gray-500 mb-4">These certifications will auto-populate in deal intake forms.</p>

                <div className="grid md:grid-cols-2 gap-6">
                  {/* Left Column - Entity Type */}
                  <div className="space-y-4">
                    <div>
                      <label className="block text-sm font-medium text-gray-300 mb-1">Entity Type</label>
                      <select
                        value={orgEntityType}
                        onChange={(e) => canEditOrg && setOrgEntityType(e.target.value)}
                        disabled={!canEditOrg}
                        className={`w-full px-3 py-2 border rounded-lg text-gray-100 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 ${
                          canEditOrg ? 'bg-gray-800 border-gray-700' : 'bg-gray-800/50 border-gray-700/50 cursor-not-allowed'
                        }`}
                      >
                  ...
                  {/* Right Column - Ownership Certifications */}
                  <div className="space-y-3">
                    <label className="flex items-center gap-3 cursor-pointer group">
                      <input
                        type="checkbox"
                        checked={lowIncomeOwned}
                        onChange={(e) => canEditOrg && setLowIncomeOwned(e.target.checked)}
                        disabled={!canEditOrg}
                        className="w-5 h-5 text-indigo-600 bg-gray-800 border-gray-700 rounded focus:ring-indigo-500"
                      />
                      <div>
                        <span className="text-sm text-gray-300 group-hover:text-white">Low-Income Community Representation</span>
                        <p className="text-xs text-gray-500">&gt;50% board/ownership from LIC</p>
                      </div>
                    </label>
                    ...
                    <label className="flex items-center gap-3 cursor-pointer group">
                      <input
                        type="checkbox"
                        checked={womanOwned}
                        onChange={(e) => canEditOrg && setWomanOwned(e.target.checked)}
                        disabled={!canEditOrg}
                        className="w-5 h-5 text-indigo-600 bg-gray-800 border-gray-700 rounded focus:ring-indigo-500"
                      />
                      <div>
                        <span className="text-sm text-gray-300 group-hover:text-white">Woman-Owned</span>
                    ...
                    <label className="flex items-center gap-3 cursor-pointer group">
                      <input
                        ...
                        onChange={(e) => canEditOrg && setMinorityOwned(e.target.checked)}
                        disabled={!canEditOrg}
                        className="w-5 h-5 text-indigo-600 bg-gray-800 border-gray-700 rounded focus:ring-indigo-500"
                      />
                      <div>
                        <span className="text-sm text-gray-300 group-hover:text-white">Minority-Owned</span>
                    ...
                    <label className="flex items-center gap-3 cursor-pointer group">
                      <input
                        ...
                        onChange={(e) => canEditOrg && setVeteranOwned(e.target.checked)}
                        disabled={!canEditOrg}
                        className="w-5 h-5 text-indigo-600 bg-gray-800 border-gray-700 rounded focus:ring-indigo-500"
                      />
                      <div>
                        <span className="text-sm text-gray-300 group-hover:text-white">Veteran-Owned</span>
      ...
      {inviteSuccess && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div className="absolute inset-0 bg-black/70" onClick={() => setInviteSuccess(null)} />
          <div className="relative bg-gray-900 rounded-xl w-full max-w-md mx-4 shadow-xl border border-gray-800 p-8 text-center">
            <div className={`w-16 h-16 ${inviteSuccess.emailSent ? 'bg-green-900/30 border-green-500/30' : 'bg-amber-900/30 border-amber-500/30'} border rounded-full flex items-center justify-center mx-auto mb-4`}>
            ...
            <h3 className="text-xl font-bold text-white mb-2">
              {inviteSuccess.emailSent ? 'Team Invite Sent!' : 'Member Added'}
            </h3>
            <p className="text-gray-400 mb-1">
              {inviteSuccess.emailSent
                ? <>Invitation email delivered to <span className="text-white font-medium">{inviteSuccess.name}</span></>
                : <>Account created for <span className="text-white font-medium">{inviteSuccess.name}</span></>
              }
            </p>
            <p className="text-sm text-gray-500">Sent to: {inviteSuccess.email}</p>
            {!inviteSuccess.emailSent && inviteSuccess.error && (
              <p className="text-sm text-amber-400 mt-3">{inviteSuccess.error}</p>
            )}
            <button
              type="button"
