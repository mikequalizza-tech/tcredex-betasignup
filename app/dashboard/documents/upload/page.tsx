...
function DocumentUploadPageContent() {
  ...
  const prefilledEntityType = searchParams?.get(
    "entityType",
  ) as EntityType | null;
  const prefilledEntityId = searchParams?.get("entityId");
  const _prefilledProjectId = searchParams?.get("projectId");

  const [step, setStep] = useState<"select" | "upload">(
    prefilledEntityType ? "upload" : "select",
  );
  const [selectedEntityType, setSelectedEntityType] = useState<EntityType>(
    prefilledEntityType || "organization",
  );
  ...
  useEffect(() => {
    if (selectedEntityId && selectedEntityType !== "user") {
      const entities =
        demoEntities[selectedEntityType as keyof typeof demoEntities] || [];
  ...
  useEffect(() => {
    if (prefilledEntityType && prefilledEntityId) {
      const entities =
        demoEntities[prefilledEntityType as keyof typeof demoEntities] || [];
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-3">
                {(["organization", "project", "deal"] as EntityType[]).map(
                  (type) => (
                    <button
                      key={type}
                      onClick={() => {
            ...
            {selectedEntityType !== "user" && (
              <div>
                ...
                >
                  ...
                  {demoEntities[
                    selectedEntityType as keyof typeof demoEntities
                  ]?.map((entity) => (
