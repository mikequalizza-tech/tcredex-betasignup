...
export async function fetchCDEs(): Promise<CDEDealCard[]> {
  ...
  try {
    ...
    return data.map((cde: CDERow) => {
      ...
      const targetSectors = Array.isArray(cde.target_sectors) && cde.target_sectors.length > 0
        ? cde.target_sectors
        : [];

      // Parse serviceAreaType from raw text if not explicitly set
      const rawServiceAreaType = cde.service_area_type || '';
      const inferredServiceAreaType = rawServiceAreaType
        || (serviceArea.toLowerCase().includes('national') ? 'national'
          : serviceArea.toLowerCase().includes('multi-state') ? 'regional'
          : serviceArea.toLowerCase().includes('statewide') || serviceArea.toLowerCase().includes('territory') ? 'state'
          : serviceArea.toLowerCase().includes('local') || serviceArea.toLowerCase().includes('county') ? 'local'
          : '');

      // Parse predominantFinancing from predominantMarket if not explicitly set
      const rawFinancing = cde.predominant_financing || '';
      const inferredFinancing = rawFinancing
        || (predominantMarket.toLowerCase().includes('real estate') ? 'Real Estate Financing'
          : predominantMarket.toLowerCase().includes('business') || predominantMarket.toLowerCase().includes('operating') ? 'Operating Business Financing'
          : '');

      // Build specialFocus from boolean flags if not provided as array
      const specialFocus: string[] = Array.isArray(cde.special_focus) && cde.special_focus.length > 0
        ? cde.special_focus
        : [
      ...
      return {
        ...
        yearAllocations: Array.isArray(cde.year_allocations) ? (cde.year_allocations as YearAllocationRow[]).map((ya) => ({
          id: ya.id || '',
          ...
          underservedStates: Array.isArray(ya.underserved_states) ? ya.underserved_states : [],
        ...
        allocationYears: Array.isArray(cde.allocation_years) ? cde.allocation_years : [],
        primaryStates: Array.isArray(cde.primary_states) ? cde.primary_states : (cde.state ? [cde.state] : []),
        targetSectors: targetSectors,
        impactPriorities: Array.isArray(cde.impact_priorities) && cde.impact_priorities.length > 0
          ? cde.impact_priorities
          : targetSectors,
        serviceArea: serviceArea,
        serviceAreaType: inferredServiceAreaType as 'national' | 'regional' | 'state' | 'local' | undefined,
        ...
        underservedStatesFocus: Boolean(cde.uts_focus || cde.underserved_states_focus),
...
export async function fetchCDEBySlug(slug: string): Promise<CDEDetail | null> {
  ...
  if (bySlug) {
    ...
    if (byId) {
      ...
      if (byOrgId) orgId = (byOrgId as Record<string, unknown>).organization_id as string;
    }
  }
...
function mapCDEToDetail(cde: CDERow): CDEDetail {
  ...
  const parseSectorsFromMarket = (market: string): string[] => {
    ...
    if (m.includes('community') || m.includes('civic')) sectors.push('community_facility');
    if (m.includes('health') || m.includes('medical') || m.includes('clinic')) sectors.push('healthcare');
    if (m.includes('education') || m.includes('school')) sectors.push('education');
    if (m.includes('housing') || m.includes('residential')) sectors.push('affordable_housing');
    if (m.includes('industrial') || m.includes('manufacturing')) sectors.push('manufacturing');
    if (m.includes('retail') || m.includes('commercial')) sectors.push('retail');
    if (m.includes('food') || m.includes('grocery')) sectors.push('food_access');
  ...
  const targetSectors = rawTargetSectors && rawTargetSectors.length > 0
    ? rawTargetSectors
  ...
  return {
    ...
    missionFocus: rawImpactPriorities && rawImpactPriorities.length > 0
      ? rawImpactPriorities
      : targetSectors,
    projectTypes: targetSectors,
    serviceArea: rawPrimaryStates || [],
    serviceAreaType: (cde.service_area_type || cde.service_area || 'Regional') as string,
    availableAllocation: Number(cde.remaining_allocation || cde.amount_remaining) || 0,
    totalAllocation: Number(cde.total_allocation) || 0,
    allocationYear: (cde.allocation_year as string) || year?.toString() || new Date().getFullYear().toString(),
    ...
    primaryContact: (cde.primary_contact_name || cde.contact_name || '') as string,
    contactEmail: (cde.primary_contact_email || cde.contact_email || '') as string,
    contactPhone: (cde.primary_contact_phone || cde.contact_phone || '') as string,
    // Additional fields from cdes_merged
    allocationType: (cde.allocation_type || 'Federal') as string,
    amountRemaining: Number(cde.amount_remaining || cde.remaining_allocation) || 0,
    nonMetroCommitment: Number(cde.non_metro_commitment) || 0,
    ...
    allocations: (cde.allocations || cde.cde_allocations || []) as CDEAllocation[],
  };
}
...
export async function fetchCDEPipelineDeals(cdeOrgId: string): Promise<Deal[]> {
  ...
  for (const row of (mergedRows || []) as Array<{ id: string; organization_id: string }>) {
  ...
  const invitedDealIds = [...new Set(
    ((matchRequests || []) as Array<{ deal_id: string }>).map(r => r.deal_id)
  )];
  ...
  return (data || []).map((deal: DealRow) => {
    ...
    return {
      ...
      sponsorName: (deal.sponsor_organization_name || sponsors?.organization_name || deal.sponsor_name || 'Unknown Sponsor') as string,
      sponsorDescription: '',
      ...
      allocation: Number(deal.nmtc_financing_requested) || Number(deal.fed_nmtc_allocation_request) || 0,
      creditPrice: 0.76,
      ...
      coordinates: deal.latitude && deal.longitude ? [deal.longitude as number, deal.latitude as number] : undefined,
  ...
  try {
    ...
    return (cdeRows as Array<Record<string, unknown>>)
      ...
      .map(cde => ({
        ...
        year: ((cde.allocation_year || cde.year || new Date().getFullYear()) as string | number).toString(),
