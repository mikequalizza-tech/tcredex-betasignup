...
export const DD_CATEGORIES: DDCategoryMeta[] = [
  // Core Project Documents
  {
    category: 'pro_forma',
    label: 'Pro Forma',
    description: 'Financial projections including income, expenses, and cash flow',
    programs: ['all'],
...
export function getCategoriesForPrograms(
  ...
  return DD_CATEGORIES.filter(
    (cat) =>
      ...
      cat.programs.some((p) => programs.includes(p as 'NMTC' | 'HTC' | 'LIHTC'))
  );
}
 ...
 */
export function getStatusColor(status: DDDocumentStatus): 'green' | 'yellow' | 'red' | 'gray' {
  switch (status) {
    case 'approved':
    case 'waived':
      return 'green';
