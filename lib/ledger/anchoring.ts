...
export async function anchorToGitHubGist(
  ...
  if (!latest) {
    return {
      ...
      error: "No ledger events to anchor",
    };
  }
...
export async function runScheduledAnchoring(config: {
  ...
  email?: {
    escrowEmail: string;
    smtp: Parameters<typeof anchorToEscrowEmail>[1];
  };
  ...
  if (config.github) {
    const result = await anchorToGitHubGist(
      config.github.token,
      config.github.gistId,
    );
    results.push(result);
  }

  if (config.email) {
    const result = await anchorToEscrowEmail(
      config.email.escrowEmail,
      config.email.smtp,
    );
    results.push(result);
  }
