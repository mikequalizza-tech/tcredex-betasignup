...
function isDealInUnderservedState(dealState: string, allocationYears?: number[]): boolean {
...
function passesFinancing(deal: DealCriteria, cde: CDEDealCard): boolean {
  ...
  if (deal.isRealEstate === undefined || deal.isRealEstate === null) return true;
...
export function calculateCDEMatchScore(cde: CDEDealCard, deal: DealCriteria): MatchResult {
  ...
  const isNational = serviceType === 'national' || (market ? /\bnational\b/.test(market) : false);
  ...
  if (cde.predominantFinancing) {
    reasons.push(`Financing: ${cde.predominantFinancing.includes('Real Estate') ? 'Real Estate' : 'Business'}`);
  }
  ...
  scores.severelyDistressed = (!cde.requireSeverelyDistressed || deal.severelyDistressed) ? 1 : 0;
  ...
  const isUts = deal.isUts || isDealInUnderservedState(deal.state, cde.allocationYears);
  ...
  if (cde.nonprofitPreferred && deal.isNonProfit) reasons.push('Nonprofit preferred match');

  // 12. Owner Occupied
  const dealOwnerOccupied = deal.isOwnerOccupied ?? true; // default true when not specified
  scores.ownerOccupied = (!cde.ownerOccupiedPreferred || dealOwnerOccupied) ? 1 : 0;
