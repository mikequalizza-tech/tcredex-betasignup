...
const BASE_SECTIONS: SectionDef[] = [
  ...
  { id: 'basics', tier: 1, isComplete: (data) => !!(data.projectName && data.sponsorName && data.projectType) },
  { id: 'location', tier: 1, isComplete: (data) => !!(data.address) },
  { id: 'programs', tier: 1, isComplete: (data) => !!(data.programs && Array.isArray(data.programs) && (data.programs as unknown[]).length > 0) },
  { id: 'costs', tier: 1, isComplete: (data) => !!(data.totalProjectCost) },

  // TIER 2 - Project Profile Ready
  { id: 'impact', tier: 2, isComplete: (data) => !!(data.communitySupport || data.communityImpact) },
  { id: 'benefits', tier: 2, isComplete: (data) => data.permanentJobsFTE !== undefined || data.constructionJobsFTE !== undefined },
  { id: 'team', tier: 2, isComplete: (data) => !!(data.ownersRepresentative) },
  { id: 'capital', tier: 2, isComplete: (data) => {
    const sources = data.financingSources;
    return Array.isArray(sources) && sources.length > 0;
  }},
  { id: 'site', tier: 2, isComplete: (data) => !!data.siteControl },
  { id: 'timeline', tier: 2, isComplete: (data) => !!data.constructionStartDate || !!data.targetClosingDate },

  // TIER 3 - Due Diligence Ready
  { id: 'readiness', tier: 3, isComplete: (data) => data.phaseIEnvironmental === 'Complete' || data.zoningApproval === 'Complete' },
  { id: 'documents', tier: 3, isComplete: (data) => {
...
const PROGRAM_SECTIONS: SectionDef[] = [
  // NMTC
  { id: 'nmtc_qalicb', tier: 2, program: 'NMTC', isComplete: (data) => {
    ...
    return grossIncome >= 50 && tangibleProperty >= 40 && employeeServices >= 40 && data.isProhibitedBusiness === false;
  }},
  // HTC
  { id: 'htc_status', tier: 2, program: 'HTC', isComplete: (data) => !!data.historicStatus && data.historicStatus !== 'none' },
  // LIHTC
  { id: 'lihtc_housing', tier: 2, program: 'LIHTC', isComplete: (data) => data.totalUnits !== undefined && data.affordableUnits !== undefined },
  // OZ
  { id: 'oz_eligibility', tier: 2, program: 'OZ', isComplete: (data) => !!data.ozInvestmentDate },
];
 ...
 */
export function calculateUnifiedReadiness(data: Record<string, unknown>, programs?: ProgramType[]): ReadinessResult {
  // Get programs from data if not provided
  const selectedPrograms: ProgramType[] = programs ||
  ...
  const applicableSections = [
    ...BASE_SECTIONS,
    ...PROGRAM_SECTIONS.filter(s => s.program && selectedPrograms.includes(s.program as ProgramType))
  ];
  ...
  const currentTier = tier3Pct >= 85 ? 3 : tier2Pct >= 70 ? 2 : tier1Pct >= 100 ? 1 : 0;
  ...
  const tier: ReadinessResult['tier'] =
    readinessScore >= 80 ? 'shovel-ready' :
    readinessScore >= 60 ? 'advanced' :
    readinessScore >= 40 ? 'developing' : 'early';
