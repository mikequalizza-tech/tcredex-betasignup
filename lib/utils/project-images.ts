...
export function deriveImageNameFromUrl(url: string, fallbackIndex?: number): string {
  try {
...
export function normalizeProjectImage(
  ...
  if (typeof image === 'object') {
    ...
    const rawUrl = typeof img.url === 'string'
      ? img.url
    ...
    const size =
      typeof img.size === 'number' && Number.isFinite(img.size)
        ? img.size
        : 0;
...
export function normalizeProjectImages(images: unknown): NormalizedProjectImage[] {
