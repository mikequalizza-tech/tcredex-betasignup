...
const s = StyleSheet.create({
  ...
  headerBrand: { fontSize: 8, color: COLORS.primaryLight, marginBottom: 4, letterSpacing: 1 },
  ...
  statLabel: { fontSize: 7, color: COLORS.textLight, textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 2 },
  statValue: { fontSize: 14, fontWeight: 'bold', color: COLORS.primary },
  statValueGreen: { fontSize: 14, fontWeight: 'bold', color: COLORS.accentDark },

  // Detail pills row
  detailRow: { flexDirection: 'row', gap: 6, marginBottom: 12, flexWrap: 'wrap' },
  detailPill: {
    backgroundColor: COLORS.bgLight,
    paddingHorizontal: 8,
    paddingVertical: 4,
  ...
  tableValue: { width: 80, fontSize: 8, fontWeight: 'bold', color: COLORS.primary, textAlign: 'right' },
  tableHeader: { flexDirection: 'row', paddingVertical: 3, borderBottomWidth: 1, borderBottomColor: COLORS.primary },
  tableHeaderText: { fontSize: 7, fontWeight: 'bold', color: COLORS.textLight, textTransform: 'uppercase' },
  ...
  demoLabel: { fontSize: 6, color: COLORS.textLight, textTransform: 'uppercase', marginBottom: 1 },
...
function ProfileDocument({ data }: { data: ProfilePdfData }) {
  ...
  const hasDemographics = data.povertyRate != null || data.medianIncome != null || data.unemployment != null;
  const hasSourcesUses = (data.sources?.length || 0) > 0 || (data.uses?.length || 0) > 0;
  const hasJobs = (data.jobsCreated || 0) > 0 || (data.constructionJobs || 0) > 0 || (data.permanentJobs || 0) > 0;
  const certs: string[] = [];
    ...
    <Document title={`${data.projectName} - Project Profile`} author="tCredex">
      <Page size="LETTER" style={s.page}>
        ...
        <View style={s.headerBar}>
          <Text style={s.headerBrand}>tCredex.com  {'\u2022'}  PROJECT PROFILE</Text>
        ...
        <View style={s.content}>
          ...
          <View style={s.statsRow}>
            <View style={s.statBox}>
              ...
              <Text style={s.statValue}>{fmtCurrency(data.totalProjectCost)}</Text>
            </View>
            <View style={s.statBoxAccent}>
              <Text style={s.statLabel}>NMTC Allocation Request</Text>
              <Text style={s.statValueGreen}>{fmtCurrency(data.nmtcRequest)}</Text>
            </View>
            {data.financingGap ? (
              <View style={s.statBox}>
                <Text style={s.statLabel}>Financing Gap</Text>
                <Text style={s.statValue}>{fmtCurrency(data.financingGap)}</Text>
              </View>
            ) : null}
          </View>
          ...
          <View style={s.detailRow}>
            ...
            {data.stateNmtcRequest ? (
              <View style={s.detailPill}>
                ...
                <Text style={s.detailPillValue}>{fmtCurrency(data.stateNmtcRequest)}</Text>
              </View>
            ) : null}
            {data.htcAmount ? (
              <View style={s.detailPill}>
                <Text style={s.detailPillLabel}>HTC Amount:</Text>
                <Text style={s.detailPillValue}>{fmtCurrency(data.htcAmount)}</Text>
              </View>
            ) : null}
          </View>
            ...
            <View style={s.demoRow}>
              ...
              {data.medianIncome != null && (
                <View style={s.demoItem}>
                  ...
                  <Text style={s.demoValue}>{data.medianIncome.toFixed(1)}%</Text>
                </View>
              )}
              ...
              {hasJobs && (
                <View style={s.demoItem}>
                  ...
                  <Text style={s.demoValue}>{data.jobsCreated || ((data.constructionJobs || 0) + (data.permanentJobs || 0))}</Text>
                </View>
              )}
            </View>
          )}
                ...
                {(data.sources?.length || 0) > 0 && (
                  <View style={{ flex: 1 }}>
                    <View style={s.tableHeader}>
                      <Text style={[s.tableHeaderText, { flex: 1 }]}>Source</Text>
                    ...
                    {data.sources!.map((src, i) => (
                      <View key={`s${i}`} style={s.tableRow}>
                        ...
                        <Text style={s.tableValue}>{fmtCurrency(src.amount)}</Text>
                      </View>
                    ))}
                  </View>
                )}
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    {data.uses!.map((use, i) => (
                      <View key={`u${i}`} style={s.tableRow}>
                        ...
                        <Text style={s.tableValue}>{fmtCurrency(use.amount)}</Text>
                      </View>
                    ))}
                  </View>
                )}
          ...
          <View style={s.section}>
            ...
            <Text style={s.body}>{truncate(cleanText(data.description), maxChars)}</Text>
          </View>
          ...
          {data.communityImpact && (
            <View style={s.section}>
              ...
              <Text style={s.body}>{truncate(cleanText(data.communityImpact), maxChars)}</Text>
            </View>
          )}
          ...
          <View style={s.section}>
            ...
            <Text style={s.body}>{truncate(cleanText(data.aboutSponsor), Math.min(maxChars, 400))}</Text>
            {certs.length > 0 && (
        ...
        <View style={s.footer}>
          <Text style={s.footerText}>
            {'\u00a9'} {new Date().getFullYear()} tCredex  {'\u2022'}  AI-Powered Tax Credit Marketplace  {'\u2022'}  tCredex.com
 ...
 */
export async function generateProfilePdfBuffer(data: ProfilePdfData): Promise<Buffer> {
