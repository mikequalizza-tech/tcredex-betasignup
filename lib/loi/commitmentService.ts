...
export class CommitmentService {
  ...
  async sendForAcceptance(id: string, userId: string): Promise<Commitment> {
    ...
    const _requiresCDE = commitment.credit_type === 'NMTC' && !!commitment.cde_id;
  ...
  async sponsorAccept(id: string, sponsorId: string, _notes?: string): Promise<Commitment> {
    const commitment = await this.getById(id);
    if (!commitment) throw new Error('Commitment not found');
  ...
  async cdeAccept(id: string, cdeUserId: string, _notes?: string): Promise<Commitment> {
    const commitment = await this.getById(id);
    if (!commitment) throw new Error('Commitment not found');
