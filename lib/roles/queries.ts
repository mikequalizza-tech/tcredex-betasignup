 ...
 */
async function fetchMarketplaceForSponsorViaAPI(_sponsorOrgId?: string): Promise<MarketplaceResult> {
  try {
    ...
    const invResult = await fetchApi<{ investors?: InvestorRow[] }>('/api/investors');
    const invJson = invResult.success && invResult.data ? invResult.data : { investors: [] };

    const investors: InvestorCard[] = (invJson.investors || []).map((inv: InvestorRow) => ({
      id: inv.id,
 ...
 * Uses authenticated API routes
 */
async function fetchMarketplaceForCDEViaAPI(_cdeOrgId?: string): Promise<MarketplaceResult> {
  try {
    const result = await fetchApi<{ deals?: DealRow[] }>('/api/deals');
    const json = result.success && result.data ? result.data : { deals: [] };
    
    const deals: Deal[] = (json.deals || []).map((deal: DealRow) => mapDealFromDB(deal));
 ...
 * Uses authenticated API routes
 */
async function fetchMarketplaceForInvestorViaAPI(_investorOrgId?: string): Promise<MarketplaceResult> {
  try {
    const result = await fetchApi<{ deals?: DealRow[] }>('/api/deals');
    const json = result.success && result.data ? result.data : { deals: [] };
...
async function fetchPublicMarketplaceViaAPI(): Promise<MarketplaceResult> {
  try {
    ...
    const result = await fetchApi<{ deals?: DealRow[] }>('/api/deals', { requireAuth: false });
    const json = result.success && result.data ? result.data : { deals: [] };
...
function mapDealFromDB(deal: DealRow): Deal {
  ...
  const sponsorDescription =
    sponsorDescriptionCandidates
      .map(normalizeText)
      .find((value) => value && value.toLowerCase() !== projectDescription.toLowerCase()) || '';

  // Determine completion date from various possible fields
  const timeline = deal.timeline as Array<{ milestone?: string; date?: string }> | undefined;
  const completionDate = (deal.projected_completion_date ||
                        deal.estimated_completion ||
                        deal.completion_date ||
                        timeline?.find((t) => t.milestone?.includes('Completion'))?.date) as string | undefined;
  ...
  return {
    ...
    sponsorName: (deal.sponsor_organization_name || deal.sponsor_name || sponsors?.organization_name || 'Unknown Sponsor') as string,
    sponsorDescription,
    ...
    timeline: [
      { milestone: 'Construction Start', date: (deal.construction_start_date || 'TBD') as string, completed: !!deal.construction_start_date },
      { milestone: 'Projected Completion', date: (completionDate || 'TBD') as string, completed: false },
...
function _calculateMatchScore(deal: Deal, cdePreferences: CDEPreferences): number {
