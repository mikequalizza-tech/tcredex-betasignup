...
interface DiscordSocketContextValue {
  ...
  onTyping: (
    handler: (data: TypingUser & { isTyping: boolean }) => void,
  ) => () => void;
}
...
const DiscordSocketContext = createContext<DiscordSocketContextValue | null>(
  null,
);
...
export function useDiscordSocketContext() {
  ...
  if (!context) {
    throw new Error(
      "useDiscordSocketContext must be used within a DiscordSocketProvider",
    );
  }
  return context;
}
