 ...
 */
export async function resolveCensusTract(
  address: string,
): Promise<TractResolution> {
  try {
 ...
 */
export async function getTractEligibility(
  tractId: string,
): Promise<TractEligibility> {
  try {
    const res = await fetch(
      `/api/eligibility?tract=${encodeURIComponent(tractId)}`,
    );
    if (!res.ok) {
    ...
    return {
      ...
      severely_distressed:
        data.programs?.includes("Severely Distressed") || false,
 ...
 */
export async function resolveAddressToEligibility(
  address: string,
): Promise<FullTractData> {
 ...
 */
export async function resolveCoordinatesToTract(
  lat: number,
  lng: number,
): Promise<TractResolution> {
  try {
    const res = await fetch(`/api/geo/resolve-tract?lat=${lat}&lng=${lng}`);
    if (!res.ok) {
 ...
 */
export async function getTractGeometry(
  tractId: string,
): Promise<GeoJSON.Feature | null> {
  const res = await fetch(
    `/api/geo/tract-geometry?tract=${encodeURIComponent(tractId)}`,
  );
  if (!res.ok) {
    return null;
  }
