...
export function computePricing(inputs: PricingInputs): PricingTier[] {
  const avgPrint =
    inputs.market.recent_prints.length > 0
      ? inputs.market.recent_prints.reduce((sum, p) => sum + p.price, 0) /
        inputs.market.recent_prints.length
      : 0.78;

  const riskAdj =
    (1 - inputs.risk.construction_risk) * inputs.risk.sponsor_track_record;
