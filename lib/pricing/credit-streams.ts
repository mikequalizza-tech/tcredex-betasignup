 ...
 */
export const MARKET_PRICE_RANGES: Record<
  CreditProgram,
  { low: number; mid: number; high: number }
> = {
  NMTC: { low: 0.75, mid: 0.8, high: 0.85 },
  HTC: { low: 0.88, mid: 0.92, high: 0.96 },
  LIHTC_9: { low: 0.88, mid: 0.93, high: 0.98 },
  LIHTC_4: { low: 0.85, mid: 0.9, high: 0.95 },
  OZ: { low: 0.85, mid: 0.9, high: 0.95 },
  BROWNFIELD: { low: 0.7, mid: 0.8, high: 0.9 }, // State dependent
};
 ...
 */
export function getPriceRange(program: CreditProgram): {
  low: number;
  mid: number;
  high: number;
} {
 ...
 */
export function getIndicativePricing(
  program: CreditProgram,
  basis: number,
  isDDA = false,
): {
  ...
  return {
    low: calculateCreditPricing({
      program,
      eligibleBasis: basis,
      creditPrice: range.low,
      isDDA,
    }),
    mid: calculateCreditPricing({
      program,
      eligibleBasis: basis,
      creditPrice: range.mid,
      isDDA,
    }),
    high: calculateCreditPricing({
      program,
      eligibleBasis: basis,
      creditPrice: range.high,
      isDDA,
    }),
  };
}
