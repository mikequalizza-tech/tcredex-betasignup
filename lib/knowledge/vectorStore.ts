...
/** A row returned from Supabase REST API */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
type SupabaseRestRow = any;
 ...
 */
async function supabaseRPC(functionName: string, params: Record<string, unknown>): Promise<SupabaseRestRow[]> {
...
export async function storeDocument(doc: DocumentMetadata): Promise<DocumentMetadata> {
  // Map camelCase to snake_case for Supabase REST API
  const result = await supabaseQuery('knowledge_documents', 'POST', {
    id: doc.id,
...
export async function getChunksForDocument(documentId: string): Promise<DocumentChunk[]> {
  ...
  return result.map((r) => ({
    ...
    metadata: r.metadata as DocumentChunk['metadata'],
