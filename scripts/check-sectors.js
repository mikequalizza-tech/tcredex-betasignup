...
const withSectors = data.filter(d => d.target_sectors && d.target_sectors.length > 0);
const without = data.filter(d => !d.target_sectors || d.target_sectors.length === 0);
...
if (without.length > 0) {
  ...
  Object.entries(finDist).sort((a, b) => b[1] - a[1]).forEach(([v, c]) => console.log(`  (${c}) ${v}`));
}
console.log('\nSector distribution:');
const sectorCounts = {};
data.forEach(d => (d.target_sectors || []).forEach(s => sectorCounts[s] = (sectorCounts[s] || 0) + 1));
