...
async function main() {
  const { data: cdes } = await supabase.from('cdes_merged').select('*').eq('status', 'active');
  ...
  for (const c of cdes) {
    ...
    if (c.primary_states && c.primary_states.length > 0) stats.has_primary_states++;
    if (c.target_sectors && c.target_sectors.length > 0) stats.has_target_sectors++;
    if (c.rural_focus === true) stats.rural_focus_true++;
    if (c.urban_focus === false) stats.urban_focus_false++;
    if (c.require_severely_distressed === true) stats.require_severely_distressed_true++;
    if ((Number(c.min_distress_percentile) || 0) > 0) stats.min_distress_percentile_nonzero++;
    if (c.minority_focus === true) stats.minority_focus_true++;
    if (c.uts_focus === true) stats.uts_focus_true++;
    if (c.underserved_states_focus === true) stats.underserved_states_focus_true++;
    if (c.nonprofit_preferred === true) stats.nonprofit_preferred_true++;
    if (c.forprofit_accepted === false) stats.forprofit_accepted_false++;
    if (c.owner_occupied_preferred === true) stats.owner_occupied_preferred_true++;
  ...
  console.log('  require_severely_distressed=true:', stats.require_severely_distressed_true);
  console.log('  min_distress_percentile>0:', stats.min_distress_percentile_nonzero);
  console.log('  minority_focus=true:', stats.minority_focus_true);
  console.log('  uts_focus=true:', stats.uts_focus_true);
  console.log('  underserved_states_focus=true (OLD column):', stats.underserved_states_focus_true);
  ...
  console.log('  owner_occupied_preferred=true:', stats.owner_occupied_preferred_true);
  console.log('  native_american_focus=true:', stats.native_american_focus_true);
  ...
  for (const c of cdes) {
    ...
    const parts = ia.split(',').map(s => s.trim()).filter(Boolean);
  ...
  for (const c of cdes) {
    ...
    const bucket = nmc === 0 ? '0%' : nmc <= 20 ? '1-20%' : nmc <= 40 ? '21-40%' : nmc <= 60 ? '41-60%' : nmc <= 80 ? '61-80%' : '81-100%';
