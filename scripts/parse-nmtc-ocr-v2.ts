...
function parseOCRText(filePath: string): CDEEntry[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const stateMatch = line.match(
      /\b([A-Z]{2})\s+(NATIONAL|LOCAL|MULTI-STATE|STATEWIDE|\$)/,
    );
    ...
    for (let j = parts.length - 1; j >= 0; j--) {
      ...
      if (
        cityParts.length < 2 &&
        word.match(/^[A-Z.]+$/) &&
        !["LLC", "INC", "INC.", "L.L.C.", "LP", "N.A."].includes(word)
      ) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (
      line.match(/\$\d{1,3}(,\d{3})+/) &&
      line.match(/\b[A-Z]{2}\s+(NATIONAL|LOCAL|MULTI)/)
    ) {
      continue;
    }
    ...
    if (
      line.match(
        /^(COLORADO GROWTH|COMMONWEALTH|COMMUNITY DEVELOPMENT|COMMUNITY FIRST|FORWARD|FRENCH|GENESIS|FNB|FORT WAYNE)/i,
      ) ||
      line.match(
        /(FUND|LLC|INC|CORPORATION|CAPITAL|DEVELOPMENT|PARTNERS|HOUSING|COMMUNITY)$/i,
      )
    ) {
      if (
        !line.match(/^\d/) &&
        !line.match(/^NATIONAL|^LOCAL|^MULTI|^STATEWIDE|^\$/)
      ) {
    ...
    if (
      line.match(
        /^(NATIONAL|LOCAL|MULTI-STATE|STATEWIDE \(OR|TERRITORY-WIDE\))$/,
      )
    ) {
      serviceAreaLines.push(
        line
          .replace("STATEWIDE (OR", "STATEWIDE")
          .replace("TERRITORY-WIDE)", ""),
      );
      continue;
    }
...
function main() {
  ...
  console.log(
    `Consider using the official CDFI Fund awards database for complete data.`,
  );
}
