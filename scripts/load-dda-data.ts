...
console.log(
  "DATABASE_URL:",
  process.env.DATABASE_URL ? "set (using Supabase)" : "NOT SET",
);
...
async function loadMetroDDA(year: "2025" | "2026") {
  ...
  const csvContent = fs.readFileSync(
    path.join(CSV_BASE_PATH, filename),
    "utf8",
  );
  const { data } = Papa.parse<MetroDDA>(csvContent, {
    header: true,
    skipEmptyLines: true,
  });
...
async function loadNonMetroDDA(year: "2025" | "2026") {
  ...
  const { data } = Papa.parse<NonMetroDDA>(csvContent, {
    header: true,
    skipEmptyLines: true,
  });
  ...
  try {
    ...
    for (let i = 0; i < data.length; i += batchSize) {
      ...
      batch.forEach((row, idx) => {
        ...
        const cumPop = row.cumulative_population
          ? Math.round(parseFloat(String(row.cumulative_population)))
          : null;
        params.push(
          row.county_fips,
          row.cbsasub,
...
async function updateDDAFlags() {
  ...
  try {
    ...
    console.log(
      `QCT + DDA 2025 (30% boost): ${result.rows[0].qct_plus_dda_2025}`,
    );
    console.log(
      `QCT + DDA 2026 (30% boost): ${result.rows[0].qct_plus_dda_2026}`,
    );
  } finally {
    client.release();
  }
