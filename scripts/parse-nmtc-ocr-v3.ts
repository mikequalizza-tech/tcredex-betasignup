...
function parseOCRText(filePath: string): CDEEntry[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (const st of STATE_CODES) {
      const regex = new RegExp(
        `\\b${st}\\b(?=\\s+(NATIONAL|LOCAL|MULTI|STATEWIDE|\\$))`,
        "i",
      );
    ...
    if (line.includes("LOCAL") && !line.includes("LOCALLY"))
      serviceArea = "LOCAL";
    else if (line.includes("MULTI-STATE")) serviceArea = "MULTI-STATE";
    else if (line.includes("STATEWIDE") || line.includes("TERRITORY"))
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (
      ...
      line.length < 20 &&
      !line.match(
        /^(NATIONAL|LOCAL|MULTI|STATEWIDE|TERRITORY|LLC|INC|FUND|CDC|CDE)/,
      )
    ) {
      // This might be a city - we'll try to match it later
      const cleanCity = line.replace(/\./g, "").trim();
      if (
        KNOWN_CITIES.some((kc) => kc === cleanCity || kc.startsWith(cleanCity))
      ) {
  ...
  const minCount = Math.min(allocateeBlock.length, amountBlock.length);
  ...
  console.log(
    `Pass 3 aligned entries: ${entries.length - entries.filter((e) => e.state !== "XX").length} (with unknown states)`,
  );
...
function main() {
  ...
  uniqueEntries.slice(0, 15).forEach((e, i) => {
    ...
    console.log(
      `${String(i + 1).padStart(2)}. ${e.allocatee.substring(0, 45).padEnd(45)} | ${cityState.padEnd(20)} | ${e.serviceArea.padEnd(12)} | $${e.amount / 1e6}M`,
    );
  });
  ...
  const csvContent = [
    ...
    ...uniqueEntries.map(
      (e) =>
        `"${e.allocatee}","${e.city}","${e.state}","${e.serviceArea}",${e.amount}`,
    ),
  ].join("\n");
  ...
  console.log(
    `Parsed: ${uniqueEntries.length} CDEs, $${(total / 1e9).toFixed(2)}B`,
  );
  console.log(
    `Gap: ${141 - uniqueEntries.length} entries, $${((9.91e9 - total) / 1e9).toFixed(2)}B`,
  );
}
