...
function getStateInfo(state) {
  ...
  if (ABBREV_TO_NAME[upper])
    return { abbrev: upper, name: ABBREV_TO_NAME[upper] };
  if (NAME_TO_ABBREV[lower])
    return { abbrev: NAME_TO_ABBREV[lower], name: lower };
  return null;
}
...
function enrichCDE(cde) {
  ...
  const predominantFinancing = String(
    cde.predominant_financing || "",
  ).toLowerCase();
  const innovativeActivities = String(
    cde.innovative_activities || "",
  ).toLowerCase();
  ...
  if (
    !cde.primary_states ||
    !Array.isArray(cde.primary_states) ||
    cde.primary_states.length === 0
  ) {
  ...
  if (!cde.native_american_focus)
    cde.native_american_focus =
      innovativeActivities.includes("indian country") ||
      innovativeActivities.includes("tribal");
  ...
  if (!cde.uts_focus)
    cde.uts_focus =
      innovativeActivities.includes("targeting identified states") ||
      innovativeActivities.includes("underserved");

  // Derive target_sectors — values MUST match intake form SectorCategory enum
  if (
    !cde.target_sectors ||
    !Array.isArray(cde.target_sectors) ||
    cde.target_sectors.length === 0
  ) {
    const sectors = [];
    if (
      predominantFinancing.includes("community") ||
      predominantFinancing.includes("facilit")
    ) {
      sectors.push(
        "Community Facility",
        "Healthcare/Medical",
    ...
    if (
      predominantFinancing.includes("industrial") ||
      predominantFinancing.includes("manufactur")
    ) {
    ...
    if (
      predominantFinancing.includes("housing") ||
      predominantFinancing.includes("for-sale")
    ) {
    ...
    if (
      predominantFinancing.includes("operating") ||
      predominantFinancing.includes("business")
    ) {
    ...
    if (market.includes("health") || market.includes("medical"))
      sectors.push("Healthcare/Medical");
    if (market.includes("education") || market.includes("school"))
      sectors.push("Education/Schools");
    if (market.includes("food") || market.includes("grocery"))
      sectors.push("Food Access/Grocery");
    if (market.includes("child") || market.includes("daycare"))
      sectors.push("Childcare/Early Education");
    if (market.includes("senior") || market.includes("elder"))
      sectors.push("Senior Services");
...
function diagnoseScore(deal, cde) {
  ...
  log.push(
    `  min_deal_size: ${cde.min_deal_size}, max_deal_size: ${cde.max_deal_size}`,
  );
  log.push(
    `  rural_focus: ${cde.rural_focus}, urban_focus: ${cde.urban_focus}`,
  );
  ...
  log.push(
    `  innovative_activities: "${(cde.innovative_activities || "").substring(0, 80)}"`,
  );
  log.push("");
  ...
  if (serviceType === "national") {
    ...
    if (!stateInfo) {
      ...
      for (const s of cde.primary_states || []) {
        ...
        if (
          cleaned.toUpperCase() === stateInfo.abbrev ||
          cleaned.toLowerCase() === stateInfo.name
        ) {
          geoResult = "PASS";
  ...
  if (isOwnerOccupied) {
    ...
    if (!isRealEstateCde && !isBusinessCde) {
      ...
      if (ventureType.includes("real estate")) isRealEstateDeal = true;
        ...
        ventureType.includes("business") ||
        ventureType.includes("operating")
      )
        isRealEstateDeal = false;
      else if (RE_KEYWORDS.some((kw) => projectType.includes(kw)))
  ...
  const isRural =
    intakeData.isRural ||
    (deal.tract_classification || "").toLowerCase().includes("rural");
  if (isRural && cde.rural_focus) {
    scores.urbanRural = 1;
    details.urbanRural = `Deal rural + CDE rural_focus`;
  } else if (!isRural && cde.urban_focus) {
    scores.urbanRural = 1;
    details.urbanRural = `Deal urban + CDE urban_focus`;
  } else if (!cde.rural_focus && !cde.urban_focus) {
    scores.urbanRural = 1;
    details.urbanRural = `CDE has no rural/urban focus — matches all`;
  } else {
    scores.urbanRural = 0;
    details.urbanRural = `isRural=${isRural}, rural_focus=${cde.rural_focus}, urban_focus=${cde.urban_focus} — MISMATCH`;
  }

  // 4. Sector
  const dealSector = normalizeText(
    intakeData.sectorCategory || deal.project_type,
  );
  ...
  if (!dealSector) {
    ...
    for (const sector of cdeSectors) {
      if (
        sector &&
        (dealSector.includes(sector) || sector.includes(dealSector))
      ) {
        sectorMatch = true;
  ...
  scores.severelyDistressed =
    !cde.require_severely_distressed || deal.tract_severely_distressed ? 1 : 0;
  ...
  if (minDistress === 0) {
    ...
    const dealDistress =
      Number(intakeData.distressPercentile) || Number(deal.distress_score) || 0;
  ...
  const isMinorityOwned =
    intakeData.minorityOwned || intakeData.isMinorityOwned || false;
  ...
  const isUts =
    underservedList.includes(dealStateForUts) || intakeData.isUts || false;
  ...
  // 11. Entity Type
  const orgTypeText = normalizeText(
    intakeData.organizationType || intakeData.entityType,
  );
  const isNonProfit =
    orgTypeText.includes("nonprofit") ||
    orgTypeText.includes("non-profit") ||
    orgTypeText.includes("501");
  const cdeForprofitAccepted = cde.forprofit_accepted ?? true;
  if (!cdeForprofitAccepted && !isNonProfit) {
    scores.entityType = 0;
  } else {
    scores.entityType = 1;
  }
  ...
  // 13. Tribal / AIAN
  const isTribal =
    intakeData.isTribal || intakeData.isAian || deal.aian === 1 || false;
...
async function runDiagnostic(deal) {
  ...
  console.log(
    `  nmtc_request: $${((Number(deal.nmtc_financing_requested) || 0) / 1e6).toFixed(1)}M`,
  );
  console.log(`  project_type: "${deal.project_type || "NULL"}"`);
  console.log(
    `  intake_data.sectorCategory: "${intakeData.sectorCategory || "NULL"}"`,
  );
  console.log(
    `  intake_data.ventureType: "${intakeData.ventureType || "NULL"}"`,
  );
  ...
  for (let cde of cdes) {
    ...
    if (result.score === 0) {
      ...
      else if (logText.includes("ELIMINATED by Financing"))
  ...
  for (const score of sortedScores) {
    ...
    console.log(
      `  ${String(score).padStart(3)}%: ${bar} (${scoreDistribution[score]} CDEs)`,
    );
  }
  console.log(`\n  Eliminated by Geographic: ${eliminated.geographic}`);
  console.log(`  Eliminated by Financing: ${eliminated.financing}`);
  console.log(
    `  Passed both gates: ${cdes.length - eliminated.geographic - eliminated.financing}`,
  );
  ...
  if (Object.keys(byScore).some((s) => Number(s) > 0 && Number(s) < 100)) {
    console.log("\n" + "=".repeat(80));
    console.log(
      "CRITERIA THAT VARY (cause score differences among non-eliminated CDEs):",
    );
    console.log("=".repeat(80));
    ...
    if (nonZeroResults.length > 0) {
      ...
      for (const c of criteria) {
        ...
        if (zeros > 0 && ones > 0) {
          console.log(
            `  ${c.padEnd(22)} ✓=${ones} ✗=${zeros} (${Math.round((zeros / (zeros + ones)) * 100)}% fail)`,
          );
        } else if (zeros === 0) {
          console.log(
            `  ${c.padEnd(22)} ALL PASS (${ones} CDEs) — provides NO differentiation`,
          );
        } else {
