...
function parseOCRText(filePath: string): CDEEntry[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.includes("PHOTO CREDITS") || line.includes("ADDITIONAL RESOURCES"))
      continue;
    ...
    else if (line.includes("STATEWIDE") || line.includes("TERRITORY-WIDE"))
...
function main() {
  ...
  entries.slice(0, 10).forEach((e, i) => {
    console.log(
      `${i + 1}. ${e.allocatee} | ${e.city}, ${e.state} | ${e.serviceArea} | $${e.amount / 1e6}M`,
    );
  });
