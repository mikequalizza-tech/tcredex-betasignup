...
function getServiceAreaType(serviceArea) {
  ...
  if (lower.includes("statewide") || lower.includes("territory-wide"))
    return "statewide";
...
function detectFocusFlags(innovativeActivities, nonMetroCommitment) {
  ...
  return {
    ...
    native_american_focus:
      lower.includes("indian country") || lower.includes("tribal"),
    uts_focus:
      lower.includes("targeting identified states") ||
 ...
 */
function deriveTargetSectors(
  predominantFinancing,
  predominantMarket,
...
async function importToMerged() {
  ...
  if (csvArg !== -1 && process.argv[csvArg + 1]) {
    ...
    if (!fs.existsSync(csvPath)) {
      csvPath = path.join(
        __dirname,
        "../../NMTC_Allocatee_Data_with_Contact_Split.csv",
      );
    }
  }
  ...
  const col = {
    name: headers.findIndex((h) =>
      h.toLowerCase().includes("name of allocatee"),
    ),
    year: headers.findIndex((h) => h.toLowerCase().includes("year of award")),
    totalAllocation: headers.findIndex((h) =>
      h.toLowerCase().includes("total allocation"),
    ),
    amountFinalized: headers.findIndex((h) =>
      h.toLowerCase().includes("amount finalized"),
    ),
    amountRemaining: headers.findIndex((h) =>
      h.toLowerCase().includes("amount remaining"),
    ),
    nonMetro: headers.findIndex((h) => h.toLowerCase().includes("non-metro")),
    serviceArea: headers.findIndex((h) =>
      h.toLowerCase().includes("service area"),
    ),
    controllingEntity: headers.findIndex((h) =>
      h.toLowerCase().includes("controlling entity"),
    ),
    predominantFinancing: headers.findIndex((h) =>
      h.toLowerCase().includes("predominant financing"),
    ),
    predominantMarket: headers.findIndex((h) =>
      h.toLowerCase().includes("predominant market"),
    ),
    innovativeActivities: headers.findIndex((h) =>
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const targetSectors = deriveTargetSectors(
      predominantFinancing,
      predominantMarket,
      innovativeActivities,
    );
    ...
    rows.push({
      ...
      controlling_entity:
        ((v[col.controllingEntity] || "").trim() || null)?.substring(0, 255) ||
        null,
  ...
  // Top 10 by remaining
  const sortedByRemaining = [...rows].sort(
    (a, b) => b.amount_remaining - a.amount_remaining,
  );
  ...
  sortedByRemaining.slice(0, 10).forEach((r, i) => {
    ...
    console.log(
      `   Remaining: $${(r.amount_remaining / 1e6).toFixed(1)}M | Total: $${(r.total_allocation / 1e6).toFixed(1)}M`,
    );
    console.log(
      `   Contact: ${r.contact_name || "N/A"} | ${r.contact_email || "N/A"}`,
    );
    console.log(
      `   States: ${r.primary_states ? r.primary_states.join(", ") : "National"}`,
    );
    console.log("");
  });
  ...
  if (!process.argv.includes("--execute")) {
    const jsonPath = path.join(
      __dirname,
      "../../cdes-merged-import-preview.json",
    );
    fs.writeFileSync(jsonPath, JSON.stringify(rows, null, 2));
    console.log(`\nPreview saved to: ${jsonPath}`);
    console.log(`\nRun with --execute to UPSERT into cdes_merged.`);
    console.log(
      `NOTE: UPSERT on (organization_id, year) preserves platform preferences.`,
    );
    return;
  }
  ...
  const QEI_UPDATE_COLUMNS = [
    ...
    "rural_focus",
    "uts_focus",
    "small_deal_fund", // FIXED: was underserved_states_focus
    "minority_focus",
    "target_sectors", // NEW: derived from QEI data
  ...
  for (let i = 0; i < rows.length; i += BATCH_SIZE) {
    ...
    if (error) {
      ...
      for (const row of batch) {
        ...
        if (singleError) {
          console.error(
            `  Error: ${row.name} (${row.year}): ${singleError.message}`,
          );
          errors++;
        } else {
          inserted++;
