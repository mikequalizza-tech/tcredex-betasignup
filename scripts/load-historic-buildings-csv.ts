...
function parseDate(value: string): string | undefined {
  if (!value || value.trim() === "" || value === "False" || value === "TRUE")
    return undefined;
...
async function loadHistoricBuildings() {
  ...
  const csvPath =
    "C:/tcredex.com/tracts/national-register-everything_20250624.csv";
  ...
  for (let i = 1; i < lines.length; i += BATCH_SIZE) {
    ...
    for (const line of batchLines) {
      try {
        ...
        const stateAbbr = state
          ? STATE_ABBREVS[state.toUpperCase()]
          : undefined;
        ...
        records.push({
          ...
          restricted_address: parseBool(
            cols[headerIndex["Restricted Address"]],
          ),
          acreage: isNaN(acreage!) ? undefined : acreage,
          area_of_significance: cleanString(
            cols[headerIndex["Area of Significance"]],
          ),
          ...
          periods_of_significance: cleanString(
            cols[headerIndex["Periods of Significance"]],
          ),
    ...
    if (
      (i + BATCH_SIZE) % 5000 < BATCH_SIZE ||
      i + BATCH_SIZE >= lines.length
    ) {
      console.log(
        `Progress: ${Math.min(i + BATCH_SIZE, lines.length - 1)}/${lines.length - 1} (${processed} inserted, ${errors} errors, ${skipped} skipped)`,
      );
    }
  }
