...
const PROGRAMS: {
...
}[] = [
  {
    ...
    description: "39% credit over 7 years for low-income community investments",
    color: "emerald",
  },
  {
    id: "HTC",
    ...
    description:
      "20% credit for qualified rehabilitation of historic buildings",
    color: "blue",
  },
  {
    id: "LIHTC",
    label: "Low-Income Housing Tax Credit",
    description: "9% or 4% credit for affordable rental housing development",
    color: "purple",
  },
  {
    id: "OZ",
    label: "Opportunity Zone",
    description: "Capital gains deferral and exclusion for designated areas",
    color: "amber",
  },
];
...
export function ProgramSelector({
  ...
  const toggleProgram = (programId: ProgramType) => {
    if (programs.includes(programId))
      onChange(programs.filter((p) => p !== programId));
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-xs text-gray-500 mb-3">
          Select one or both - many deals layer federal and state credits
        </p>
        <div className="flex gap-3">
          <button
            type="button"
            onClick={() => toggleLevel("federal")}
          ...
          >
            <div className="flex items-center justify-center gap-2">
              <span
                className={`w-5 h-5 rounded flex items-center justify-center text-xs border ${isFederalSelected ? "bg-indigo-500 border-indigo-500 text-white" : "border-gray-600"}`}
              >
                {isFederalSelected ? "✓" : ""}
              </span>
          ...
          >
            <div className="flex items-center justify-center gap-2">
              <span
                className={`w-5 h-5 rounded flex items-center justify-center text-xs border ${isStateSelected ? "bg-sky-500 border-sky-500 text-white" : "border-gray-600"}`}
              >
                {isStateSelected ? "✓" : ""}
              </span>
        ...
        {isFederalSelected && isStateSelected && (
          <div className="mt-3 p-3 bg-green-900/20 border border-green-500/30 rounded-lg">
            <p className="text-sm text-green-300">
              ✓ Federal + State selected - maximize your credit stack!
            </p>
          </div>
        )}
      </div>
      ...
      <div>
        <label className="block text-sm font-medium text-gray-300 mb-2">
          Select Programs <span className="text-red-400">*</span>
        </label>
        <p className="text-xs text-gray-500 mb-4">
          Select all programs you're seeking. Multiple programs can be layered.
        </p>
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <span
                    className={`px-2 py-0.5 rounded text-xs font-bold ${badgeClasses[program.color as keyof typeof badgeClasses]}`}
                  >
                    {program.id}
                  </span>
                  <span
                    className={`w-5 h-5 rounded-full flex items-center justify-center text-xs ${isSelected ? badgeClasses[program.color as keyof typeof badgeClasses] : "bg-gray-700 text-gray-500"}`}
                  >
                    {isSelected ? "✓" : ""}
                  </span>
                </div>
                <div
                  className={`font-medium mb-1 ${textClasses[program.color as keyof typeof textClasses]}`}
                >
                  {program.label}
                </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <h4 className="font-medium text-indigo-300">
                Credit Stacking Detected
              </h4>
              <p className="text-sm text-indigo-400/80 mt-1">
                You've selected multiple programs:{" "}
                <strong>{programs.join(" + ")}</strong>. Our platform will help
                optimize the capital stack across programs while ensuring
                compliance.
              </p>
            </div>
          </div>
