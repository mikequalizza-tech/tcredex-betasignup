...
const TEAM_ROLES: TeamRole[] = [
  ...
  },
  {
    ...
    required: false,
    category: "development",
    placeholder: "CM firm name (if applicable)",
  },
  {
    field: "ownersRepresentative",
  ...
  // Operations Team
  {
    ...
    required: false,
    category: "operations",
    placeholder: "Leasing firm or agent",
  },
  {
    ...
    required: false,
    category: "operations",
    placeholder: "PM company",
  },
  {
    field: "accountantReporting",
    label: "Accountant (Reporting)",
    required: false,
    category: "operations",
    placeholder: "CPA firm",
  },
  {
    field: "complianceReporter",
    label: "Compliance Reporter",
    required: false,
    category: "operations",
    placeholder: "Compliance firm or contact",
  },
  {
    field: "fundraiser",
    label: "Fundraiser / Capital Advisor",
    required: false,
    category: "operations",
  ...
  {
    field: "nmtcConsultant",
    label: "Tax Credit Consultant",
    required: false,
    category: "nmtc",
    placeholder: "Tax credit advisory firm",
  },
  {
    field: "nmtcAttorney",
    label: "Tax Credit Attorney",
    required: false,
    category: "nmtc",
    placeholder: "Legal counsel for tax credits",
  },
  {
    field: "nmtcAccountantModeler",
    label: "Tax Credit Accountant / Modeler",
    required: false,
    category: "nmtc",
    placeholder: "Financial modeling firm",
  },
];
        ...
        {roles.map((role) => (
          <div key={role.field as string}>
            ...
            <input
              ...
              onChange={(e) =>
                onChange({ [role.field]: e.target.value || undefined })
              }
...
const formatOrgType = (type?: string) => {
  ...
  const types: Record<string, string> = {
    sponsor: "Project Sponsor / Developer",
    cde: "Community Development Entity (CDE)",
    investor: "Investor",
    nonprofit: "Non-Profit Organization",
    forprofit: "For-Profit Corporation",
...
export function ProjectTeam({
  ...
  const requiredFilled = TEAM_ROLES.filter(
    (role) => role.required && data[role.field],
  ).length;
  const requiredTotal = TEAM_ROLES.filter((role) => role.required).length;

  return (
    <div className="space-y-6">
      {/* Organization Type - Static field */}
      {organizationType && (
        <div className="bg-gray-800/30 rounded-lg border border-gray-700 p-4">
          ...
          <div className="text-gray-100 font-medium">
            {formatOrgType(organizationType)}
          </div>
      ...
      <div className="flex items-center justify-between">
        <div>
          <p className="text-sm text-gray-400">
            Team members can be updated as they're confirmed. Enter "TBD" or
            "N/A" if not yet determined.
          </p>
        </div>
        <div className="text-right">
          <div className="text-2xl font-bold text-indigo-400">
            {totalFilled}/{TEAM_ROLES.length}
          </div>
      ...
      {requiredFilled < requiredTotal && (
        <div className="bg-amber-900/20 border border-amber-500/30 rounded-lg p-3 flex items-center gap-2">
          ...
          <span className="text-sm text-amber-300">
            {requiredTotal - requiredFilled} required team member
            {requiredTotal - requiredFilled > 1 ? "s" : ""} not yet specified
          </span>
        </div>
      )}
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 mt-1">
                Having experienced team members identified increases CDE
                confidence in project execution. CDEs review sponsor track
                record and team qualifications as part of due diligence.
              </p>
            </div>
          </div>
        </div>
      ...
      {(data.programs?.includes("NMTC") ||
        data.programs?.includes("LIHTC") ||
        data.programs?.includes("HTC")) &&
        !data.nmtcConsultant &&
        !data.nmtcAttorney && (
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-gray-300 font-medium text-sm">
                  Tax Credit Team Recommendation
                </p>
                <p className="text-xs text-gray-400 mt-1">
                  Tax credit transactions are complex. Most sponsors engage
                  specialized tax credit consultants and attorneys. If you need
                  referrals, tCredex can connect you with experienced
                  professionals.
                </p>
              </div>
            </div>
