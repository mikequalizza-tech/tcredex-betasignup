...
import {
  Building2, TrendingUp, ChevronDown, ChevronUp, Target, Briefcase,
  DollarSign, FileText, Search, MapPin,
  Settings, Filter, ExternalLink, Map
} from 'lucide-react';
import { fetchApi } from '@/lib/api/fetch-utils';
import DDVaultSummaryWidget from './widgets/DDVaultSummary';
import DealStatusTimeline, { mapDealStatusToStage } from './widgets/DealStatusTimeline';
...
export default function CDEDashboardV2({
  ...
  const [activeTab, setActiveTab] = useState<'queue' | 'pipeline' | 'mandate' | 'uts'>('queue');
  ...
  async function loadDashboardData() {
    ...
    try {
      ...
      const [autoMatchResult, pipelineResult, allocResult] = await Promise.all([
        ...
        organizationId
          ? fetch(`/api/automatch?scan=true&cdeId=${organizationId}&minScore=0`, { credentials: 'include' })
              .then(r => r.ok ? r.json() : null)
              .catch(() => null)
          : Promise.resolve(null),
        // 2. Pipeline deals (outreach-invited + assigned)
        organizationId
          ? fetchApi<{ deals: RawCDEDeal[] }>(`/api/deals/by-organization?orgId=${organizationId}&orgType=cde`)
      ...
      if (autoMatchResult?.matches && Array.isArray(autoMatchResult.matches)) {
        const scored: MatchedDeal[] = autoMatchResult.matches.map((m: RawAutoMatchResult) => ({
          id: m.id,
      ...
      if (pipelineResult.success && pipelineResult.data?.deals) {
        const pipelineDeals: PipelineDeal[] = pipelineResult.data.deals.map((deal: RawCDEDeal) => ({
          id: deal.id,
          projectName: deal.project_name || 'Untitled',
          sponsorName: deal.sponsors?.organization_name || deal.sponsor_organization_name || deal.sponsor_name || 'Unknown',
      ...
      const allocs = allocResult.success && Array.isArray(allocResult.data) ? allocResult.data : [];
      if (allocs.length > 0) {
        const total = allocs.reduce((sum: number, a: RawAllocation) => sum + (a.awarded_amount || 0), 0);
        const deployed = allocs.reduce((sum: number, a: RawAllocation) => sum + (a.deployed_amount || 0), 0);
        if (total > 0) {
          setAllocation({
  ...
  const filteredQueue = tierFilter === 'all'
    ? matchQueue
    : tierFilter === 1
  ...
  const tabs = [
    { id: 'queue', label: 'Match Queue', icon: TrendingUp, count: matchQueue.length },
    { id: 'pipeline', label: 'My Pipeline', icon: Briefcase, count: pipeline.length },
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-blue-600/90 to-indigo-600/90 backdrop-blur rounded-xl p-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-blue-200 text-sm">{orgName} • Deployment Tracker</p>
        </div>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-6">
              <div className="text-center">
                <div className="text-xl font-bold text-emerald-400">{formatCurrency(allocation.deployed)}</div>
                <div className="text-xs text-gray-500">Deployed</div>
              </div>
              <div className="text-center">
                <div className="text-xl font-bold text-purple-400">{formatCurrency(allocation.reserved)}</div>
                <div className="text-xs text-gray-500">Reserved</div>
              </div>
              <div className="text-center">
                <div className="text-xl font-bold text-blue-400">{formatCurrency(allocation.available)}</div>
      ...
      <div className="grid grid-cols-4 gap-4">
        <div className="bg-gray-900/80 backdrop-blur rounded-xl border border-gray-800 p-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs text-gray-500 uppercase tracking-wider">UTS Progress</p>
              <p className="text-2xl font-bold text-white mt-1">{utsStatus.currentPercentage}%</p>
              <p className="text-xs text-gray-500">Target: {utsStatus.targetPercentage}%</p>
            </div>
            <UTSGauge current={utsStatus.currentPercentage} target={utsStatus.targetPercentage} />
          </div>
        </div>
        <div className="bg-gray-900/80 backdrop-blur rounded-xl border border-gray-800 p-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs text-gray-500 uppercase tracking-wider">Strong Matches</p>
              <p className="text-2xl font-bold text-emerald-400 mt-1">{excellentCount}</p>
            </div>
        ...
        <div className="bg-gray-900/80 backdrop-blur rounded-xl border border-gray-800 p-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs text-gray-500 uppercase tracking-wider">In Pipeline</p>
              <p className="text-2xl font-bold text-purple-400 mt-1">{pipeline.length}</p>
            </div>
        ...
        <div className="bg-gray-900/80 backdrop-blur rounded-xl border border-gray-800 p-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs text-gray-500 uppercase tracking-wider">DD Pending</p>
      ...
      <div className="min-h-[400px]">
        {activeTab === 'queue' && (
          <MatchQueueTab
            ...
            onToggleDeal={(id) => setExpandedDeal(expandedDeal === id ? null : id)}
          />
        )}
        ...
        {activeTab === 'uts' && (
          <UTSStrategyTab utsStatus={utsStatus} />
        )}
      </div>
    </div>
  );
...
function AllocationBar({ allocation }: { allocation: AllocationStatus }) {
  const deployedPct = allocation.totalAllocation > 0 ? (allocation.deployed / allocation.totalAllocation) * 100 : 0;
  const reservedPct = allocation.totalAllocation > 0 ? (allocation.reserved / allocation.totalAllocation) * 100 : 0;
...
function UTSGauge({ current, target }: { current: number; target: number }) {
  ...
  const color = percentage >= 100 ? '#10b981' : percentage >= 75 ? '#f59e0b' : '#f43f5e';
      ...
      {deals.length === 0 ? (
        <div className="bg-gray-900/80 rounded-xl border border-gray-800 p-12 text-center">
          ...
          <p className="text-gray-500 mb-4">
            No marketplace deals match your CDE criteria yet. Check back as new deals are submitted.
          </p>
          <Link
            href="/deals"
  ...
  if (deals.length === 0) {
    return (
      <div className="bg-gray-900/80 rounded-xl border border-gray-800 p-12 text-center">
        ...
        <p className="text-sm text-gray-500">When sponsors send you allocation requests, accepted deals will appear here.</p>
      </div>
    );
  }
    ...
    return (
      <div className="bg-gray-900/80 rounded-xl border border-gray-800 p-12 text-center">
        ...
        <p className="text-sm text-gray-500">
          CDFI Fund Underserved Target States tracking will be available once your allocation data is loaded.
        </p>
      </div>
    );
  }
              ...
              <div className="flex items-center gap-3">
                <div className="w-32 h-2 bg-gray-800 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{ width: `${(item.percentage / utsStatus.targetPercentage) * 100}%` }}
                  />
                </div>
                <span className="text-sm text-white w-16 text-right">{item.percentage}%</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="bg-gray-900/80 rounded-xl border border-gray-800 p-4">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-lg font-bold text-white">{utsStatus.currentPercentage}%</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-sm text-gray-400">Target UTS %</span>
            <span className="text-lg font-bold text-blue-400">{utsStatus.targetPercentage}%</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-sm text-gray-400">Gap</span>
            <span className={`text-lg font-bold ${
              utsStatus.currentPercentage >= utsStatus.targetPercentage ? 'text-emerald-400' : 'text-amber-400'
            }`}>
...
function MatchDealCard({
  ...
  const getStrengthStyle = (score: number) => {
    ...
    if (score >= 50) return 'bg-amber-900/50 text-amber-300 border-amber-500/30';
        ...
        <div className="flex items-start justify-between">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className={`px-2 py-0.5 rounded text-xs font-medium border ${getStrengthStyle(deal.matchScore)}`}>
                {getStrengthLabel(deal.matchScore)}
              </span>
              <span className="text-xs text-gray-500">{deal.city}, {deal.state}</span>
            </div>
            <h4 className="text-base font-semibold text-white">{deal.projectName}</h4>
            <p className="text-xs text-gray-500">{deal.sponsorName} • {formatCurrency(deal.allocation)} QEI</p>
          </div>
          <div className="flex items-center gap-3">
            <div className="text-right">
              <p className={`text-lg font-bold ${getScoreColor(deal.matchScore)}`}>{deal.matchScore}</p>
      ...
      {expanded && (
        <div className="border-t border-gray-800 p-4 space-y-4">
          ...
          {deal.matchReasons.length > 0 && (
            <div>
              <p className="text-xs text-gray-500 mb-2 uppercase tracking-wider">Why This Match</p>
              ...
              {deal.tractType.map(tract => (
                <span
                  ...
                  className={`px-2 py-1 rounded text-xs font-medium ${
                    tract === 'SD' ? 'bg-red-900/50 text-red-400 border border-red-700/50' :
                    tract === 'QCT' ? 'bg-blue-900/50 text-blue-400 border border-blue-700/50' :
      ...
      <div className="p-4 cursor-pointer" onClick={onToggle}>
        <div className="flex items-start justify-between mb-3">
          <div>
            ...
            <h4 className="text-base font-semibold text-white">{deal.projectName}</h4>
            <p className="text-xs text-gray-500">{deal.sponsorName} • {formatCurrency(deal.allocation)}</p>
          </div>
          <div className="flex items-center gap-3">
            <div className="text-right">
              <p className="text-lg font-bold text-white">{deal.ddCompletion || 0}%</p>
        ...
        <DealStatusTimeline currentStage={mapDealStatusToStage(deal.status)} compact />
      </div>
