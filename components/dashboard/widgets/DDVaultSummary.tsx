...
import { FileText, CheckCircle, Clock, AlertCircle, ChevronRight, FolderOpen } from 'lucide-react';
 ...
 */
export default function DDVaultSummaryWidget({ dealId, dealName, className = '' }: DDVaultSummaryWidgetProps) {
  ...
  useEffect(() => {
    async function loadVault() {
      ...
      setLoading(true);
      try {
        const result = await fetchApi<{ data: DDDocument[]; summary: DDVaultSummary }>(
  ...
  if (loading) {
    return (
      <div className={`bg-gray-900/80 backdrop-blur rounded-xl border border-gray-800 p-4 ${className}`}>
    ...
    <div className={`bg-gray-900/80 backdrop-blur rounded-xl border border-gray-800 p-4 ${className}`}>
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-sm font-semibold text-white flex items-center gap-2">
          <FileText className="w-4 h-4 text-purple-400" />
          Due Diligence
          {dealName && <span className="text-gray-500 font-normal">• {dealName}</span>}
        </h3>
        <Link
          href={`/deals/${dealId}/dd-vault`}
      ...
      <div className="border-t border-gray-800 pt-3">
        ...
        <div className="space-y-1.5">
          ...
          {documents.filter((d) => d.status !== 'not_started').length === 0 && (
            <p className="text-xs text-gray-500 py-2">No document activity yet</p>
          )}
        </div>
      </div>
    </div>
...
function CompletionGauge({ percentage }: { percentage: number }) {
  ...
  const color = percentage >= 80 ? '#10b981' : percentage >= 50 ? '#f59e0b' : '#f43f5e';
...
function StatusIndicator({ icon: Icon, count, label, color }: StatusIndicatorProps) {
  const colorClasses = {
  ...
  return (
    <div className={`rounded-lg p-2 ${colorClasses[color].split(' ')[1]}`}>
      <div className="flex items-center gap-1.5">
        ...
        <span className={`text-lg font-bold ${colorClasses[color].split(' ')[0]}`}>{count}</span>
      </div>
  ...
  return (
    <div className="flex items-center justify-between py-1 px-2 bg-gray-800/30 rounded">
      <span className="text-xs text-gray-300">
        {categoryLabels[document.category] || document.category.replace(/_/g, ' ')}
      </span>
