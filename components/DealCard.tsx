...
const generateProjectNumber = (id: string, submittedDate?: string) => {
  const year = submittedDate ? new Date(submittedDate).getFullYear() : new Date().getFullYear();
  const num = id.replace(/\D/g, '').slice(0, 3).padStart(3, '0');
  return `tcredex.com_File_${year}-${num}`;
};

export default function DealCard({ deal, onRequestMemo, memoRequested, visibilityTier: _visibilityTier = 'public', compact = false }: DealCardProps) {
    ...
    return (
      <div className="w-full bg-gray-900 rounded-xl border border-gray-800 overflow-hidden hover:border-gray-700 transition-colors">
        ...
        <div className={`h-32 bg-gradient-to-br ${
          programType === 'NMTC' ? 'from-emerald-900/50 to-teal-900/30' :
          programType === 'HTC' ? 'from-blue-900/50 to-indigo-900/30' :
          programType === 'LIHTC' ? 'from-purple-900/50 to-indigo-900/30' :
          'from-amber-900/50 to-orange-900/30'
        } flex items-center justify-center`}>
          <span className="text-5xl opacity-50">
            {programType === 'NMTC' ? '🏗️' : programType === 'HTC' ? '🏛️' : programType === 'LIHTC' ? '🏘️' : '📍'}
          </span>
        </div>
        ...
        <div className="p-4">
          {/* Program Badge */}
          <span className={`inline-block px-2 py-0.5 rounded text-xs font-bold mb-2 ${PROGRAM_BADGE_COLORS[programType]}`}>
            {programType}
          </span>
          ...
          <div className="flex justify-between items-center mb-3">
            <div>
              ...
              <p className="text-lg font-bold text-emerald-400">{formatCurrency(deal.allocation)}</p>
            </div>
    ...
    <div className="w-full max-w-sm bg-slate-900 rounded-xl border border-slate-700 overflow-hidden">
      ...
      <div className="p-4 border-b border-slate-700">
        <div className="flex justify-between items-start gap-3">
          <div className="flex-1 min-w-0">
            <h2 className="font-bold text-white text-lg leading-tight">{deal.projectName || 'Untitled'}</h2>
            <p className="text-gray-400 text-sm">{deal.city}, {deal.state}</p>
          </div>
          {/* tCredex Logo Icon - Inline SVG (will render in PDF) */}
          <svg className="flex-shrink-0 w-12 h-12" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
            <rect width="100" height="100" rx="20" fill="url(#tcredex_gradient)"/>
            <path d="M35 25H65V75H50V40H35V25Z" fill="white"/>
            <path d="M25 65L45 45H35V65H25Z" fill="white"/>
            <defs>
              <linearGradient id="tcredex_gradient" x1="0" y1="0" x2="100" y2="100" gradientUnits="userSpaceOnUse">
      ...
      <div className="p-4 space-y-1.5 text-sm">
        ...
        <div className="flex justify-between">
          ...
          <span className="text-white font-medium">{deal.sponsorName || 'N/A'}</span>
        </div>
        ...
        <div className="flex justify-between">
          ...
          <span className="text-white font-mono">{deal.censusTract || 'N/A'}</span>
        </div>
        ...
        <div className="flex justify-between">
          ...
          <span className="text-cyan-400 font-semibold">{formatCurrency(deal.projectCost)}</span>
        </div>
        ...
        {isNMTC && (
          <div className="flex justify-between">
            ...
            <span className="text-white font-semibold">{formatCurrency(deal.allocation)}</span>
          </div>
        )}
        ...
        {isNMTC && (
          <div className="flex justify-between">
            ...
            <span className="text-white">{formatCurrency(deal.stateNMTCAllocation || 0)}</span>
          </div>
        )}
        ...
        {(isHTC || deal.htcAmount) && (
          <div className="flex justify-between">
            ...
            <span className="text-white">{formatCurrency(deal.htcAmount || deal.allocation)}</span>
          </div>
        )}
        ...
        <div className="flex justify-between">
          ...
          <span className="text-orange-400 font-semibold">{formatCurrency(deal.financingGap)}</span>
        </div>
        ...
        {deal.sources && deal.sources.length > 0 && (
          <>
            <div className="h-2" />
            <div className="text-gray-500 text-xs uppercase tracking-wide underline">Sources:</div>
            <div className="text-white text-xs leading-relaxed">
              {deal.sources.map(s => s.name).join(', ')}
            </div>
            <div className="text-gray-400 text-xs italic">
              Total: {formatCurrency(deal.totalSources || deal.sources.reduce((sum, s) => sum + s.amount, 0))}
            </div>
          </>
        )}
        ...
        {deal.uses && deal.uses.length > 0 && (
          <>
            <div className="h-1" />
            <div className="text-gray-500 text-xs uppercase tracking-wide underline">Uses:</div>
            <div className="text-white text-xs leading-relaxed">
              {deal.uses.map(u => u.name).join(', ')}
            </div>
            <div className="text-gray-400 text-xs italic">
              Total: {formatCurrency(deal.totalUses || deal.uses.reduce((sum, u) => sum + u.amount, 0))}
            </div>
          </>
        )}
        ...
        <div className="flex justify-between">
          ...
          <span className="text-white font-mono text-xs">{generateProjectNumber(deal.id, deal.submittedDate)}</span>
        </div>
      ...
      <div className="px-4 py-3 bg-slate-800/50 border-t border-slate-700 text-xs">
        <div className="flex items-center justify-between">
          ...
          <svg className="w-10 h-10" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
            <rect width="100" height="100" rx="20" fill="url(#tcredex_gradient_footer)"/>
            <path d="M35 25H65V75H50V40H35V25Z" fill="white"/>
            <path d="M25 65L45 45H35V65H25Z" fill="white"/>
            <defs>
              <linearGradient id="tcredex_gradient_footer" x1="0" y1="0" x2="100" y2="100" gradientUnits="userSpaceOnUse">
                <stop stopColor="#0000FF"/>
                <stop offset="1" stopColor="#8000FF"/>
              </linearGradient>
            </defs>
      ...
      <div className="p-4 pt-2">
        {memoRequested ? (
          <button disabled className="w-full bg-green-600 text-white font-semibold py-3 rounded-lg text-sm">
            ✓ Interest Expressed
          </button>
        ) : onRequestMemo ? (
          <button
