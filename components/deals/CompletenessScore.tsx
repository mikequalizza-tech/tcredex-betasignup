...
export function CompletenessScore({
  ...
  if (variant === "compact") {
    return (
      <div className={`inline-flex items-center gap-2 ${className}`}>
        <div
          className={`px-3 py-1.5 rounded-lg ${tierDisplay.bgColor} ${tierDisplay.textColor} font-bold`}
        >
  ...
  return (
    <div
      className={`bg-gray-900 rounded-xl border border-gray-800 overflow-hidden ${className}`}
    >
      {/* Header */}
      ...
      <div className="px-6 py-4">
        <div className="h-3 bg-gray-800 rounded-full overflow-hidden">
          <div
            className={`h-full transition-all duration-500 ${
              readinessResult.percentage >= 80
                ? "bg-gradient-to-r from-green-500 to-emerald-400"
                : readinessResult.percentage >= 60
                  ? "bg-gradient-to-r from-blue-500 to-indigo-400"
                  : readinessResult.percentage >= 40
        ...
        <div className="flex justify-between text-xs text-gray-500 mt-2">
          <span>
            {readinessResult.completedFields} of {readinessResult.totalFields}{" "}
            fields
          </span>
      ...
      <div className="px-6 py-4 border-t border-gray-800">
        ...
        <div className="flex items-center gap-2">
          {[
            ...
            const isActive =
              readinessResult.percentage >= stage.min &&
              readinessResult.percentage <= stage.max;
                ...
                <div
                  className={`h-2 rounded-full ${
                    isPassed
                      ? "bg-green-500"
                      : isActive
                        ? tierDisplay.textColor.replace("text-", "bg-")
                        : "bg-gray-700"
                  }`}
                />
                <p
