...
function parseFrontmatter(fileContent: string, filename?: string) {
  try {
    ...
    if (!match) {
      console.warn(
        `[MDX] No frontmatter found in ${filename || "unknown file"}`,
      );
      return {
        metadata: {
    ...
    console.error(
      `[MDX] Error parsing frontmatter in ${filename || "unknown file"}:`,
      error,
    );
    return {
      metadata: {
...
function getMDXData(dir: string) {
  ...
  return mdxFiles
    ...
    .filter(
      (post) => post.metadata.title !== "Untitled" || post.content.length > 0,
    );
}
