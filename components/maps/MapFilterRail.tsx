...
export default function MapFilterRail({
  ...
  const { isAuthenticated, isLoading, organizationId, userEmail } =
    useCurrentUser();
    ...
    async (lng: number, lat: number) => {
      ...
      try {
        ...
        if (!tractResponse.ok || !tractData.geoid)
          throw new Error(
            tractData.error || "Could not determine census tract",
          );
        const eligibilityResponse = await fetch(
          `/api/eligibility?tract=${tractData.geoid}`,
        );
        if (requestId !== currentRequestId.current) return;
        ...
        const tract: TractData = {
          ...
          unemploymentQualifies:
            eligibilityData.federal?.unemployment_qualifies || false,
          ...
          severelyDistressed:
            eligibilityData.federal?.severely_distressed || false,
          metroStatus: eligibilityData.federal?.metro_status || "N/A",
          stateName:
            eligibilityData.location?.state || tractData.state_name || "",
          stateNmtc: eligibilityData.state?.nmtc || false,
          stateHtc: eligibilityData.state?.htc || false,
          stateBrownfield: eligibilityData.state?.brownfield || false,
        ...
        if (requestId === currentRequestId.current) setIsSearching(false);
      }
    },
    ...
    async (searchAddress?: string) => {
      ...
      try {
        const geoResponse = await fetch(
          `/api/geo/tract-lookup?address=${encodeURIComponent(addrToSearch)}`,
        );
        const geoData = await geoResponse.json();
        if (requestId !== currentRequestId.current) return;
        if (!geoResponse.ok || !geoData.geoid) {
          setSearchError(geoData.error || "Could not determine census tract");
          setTractResult(null);
          setIsSearching(false);
          return;
        }
        const eligibilityResponse = await fetch(
          `/api/eligibility?tract=${geoData.geoid}`,
        );
        if (requestId !== currentRequestId.current) return;
        if (!eligibilityResponse.ok)
          throw new Error("Eligibility lookup failed");
        ...
        const tract: TractData = {
          ...
          metroStatus: eligibilityData.federal?.metro_status || "N/A",
          stateName:
            eligibilityData.location?.state || geoData.state_name || "",
          stateNmtc: eligibilityData.state?.nmtc || false,
          stateHtc: eligibilityData.state?.htc || false,
          stateBrownfield: eligibilityData.state?.brownfield || false,
          opportunityZone: eligibilityData.federal?.opportunity_zone || false,
        ...
        if (onTractFound && geoData.coordinates)
          onTractFound(tract, geoData.coordinates, addrToSearch);
      } catch (error) {
        setSearchError(
  ...
  const toggleCreditType = (
    type: "nmtc" | "htc" | "lihtc" | "oz" | "brownfield",
  ) => {
  ...
  const updateCommunityImpact = (
    updates: Partial<FilterState["communityImpact"]>,
  ) => {
    onFiltersChange({
      ...filters,
      communityImpact: { ...filters.communityImpact, ...updates },
    });
  };
    ...
    <div className="w-80 h-full bg-gray-950 border-r border-gray-800 flex flex-col overflow-hidden">
      ...
      <div className="flex-none p-4 border-b border-gray-800 bg-gray-900/50">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-3">
            <div
              className={`w-8 h-8 ${config.color} rounded-lg flex items-center justify-center text-white`}
            >
              {config.icon}
            </div>
            <div>
      ...
      <div className="flex-1 overflow-y-auto">
          ...
          <div className="space-y-3">
            ...
            {tractResult && (
              <div className="bg-gray-800/50 rounded-lg border border-gray-700 overflow-hidden">
                {/* Eligibility Header */}
                <div
                  className={`px-3 py-3 border-b border-gray-700 ${tractResult.nmtcEligible || tractResult.opportunityZone || tractResult.lihtcQct ? "bg-green-900/30" : "bg-red-900/20"}`}
                >
                  <div className="flex items-center gap-2 mb-2">
                    <div
                      className={`w-6 h-6 rounded-full flex items-center justify-center ${tractResult.nmtcEligible || tractResult.opportunityZone || tractResult.lihtcQct ? "bg-green-500" : "bg-red-500"}`}
                    >
                    ...
                    <span
                      className={`font-bold text-sm ${tractResult.nmtcEligible || tractResult.opportunityZone || tractResult.lihtcQct ? "text-green-400" : "text-red-400"}`}
                    >
                      {tractResult.nmtcEligible ||
                      tractResult.opportunityZone ||
                      tractResult.lihtcQct
                  ...
                  <div className="flex flex-wrap gap-1">
                    ...
                    {tractResult.nmtcEligible && (
                      <span className="px-2 py-0.5 bg-green-800/50 text-green-300 text-xs rounded">
                        Federal NMTC
                      </span>
                    )}
                    {tractResult.stateNmtc && (
                      <span className="px-2 py-0.5 bg-green-700/50 text-green-200 text-xs rounded">
                        + State NMTC
                      </span>
                    )}
                    ...
                    {tractResult.lihtcQct && (
                      <span className="px-2 py-0.5 bg-purple-800/50 text-purple-300 text-xs rounded">
                        LIHTC QCT{" "}
                        {tractResult.lihtcQct2025 && tractResult.lihtcQct2026
                          ? "25/26"
                          : tractResult.lihtcQct2025
                            ? "2025"
                            : "2026"}
                      </span>
                    )}
                    ...
                    {tractResult.lihtcDda && (
                      <span className="px-2 py-0.5 bg-indigo-800/50 text-indigo-300 text-xs rounded">
                        DDA{" "}
                        {tractResult.lihtcDda2025 && tractResult.lihtcDda2026
                          ? "25/26"
                          : tractResult.lihtcDda2025
                            ? "2025"
                            : "2026"}
                      </span>
                    )}
                    ...
                    {/* Opportunity Zone */}
                    {tractResult.opportunityZone && (
                      <span className="px-2 py-0.5 bg-blue-800/50 text-blue-300 text-xs rounded">
                        Opportunity Zone
                      </span>
                    )}
                    ...
                    {tractResult.severelyDistressed && (
                      <span className="px-2 py-0.5 bg-orange-800/50 text-orange-300 text-xs rounded">
                        Severely Distressed
                      </span>
                    )}
                  </div>
                ...
                <div className="grid grid-cols-2 gap-px bg-gray-700">
                  ...
                  <div className="bg-gray-800 p-2.5">
                    ...
                    <div
                      className={`text-lg font-bold ${tractResult.incomeQualifies ? "text-green-400" : "text-white"}`}
                    >
                      {tractResult.medianIncomePct.toFixed(1)}%
                    </div>
                    {tractResult.incomeQualifies && (
                  ...
                  <div className="bg-gray-800 p-2.5">
                    ...
                    <div
                      className={`text-lg font-bold ${tractResult.povertyQualifies ? "text-amber-400" : "text-white"}`}
                    >
                      {tractResult.povertyRate.toFixed(1)}%
                    </div>
                    {tractResult.povertyQualifies && (
                  ...
                  <div className="bg-gray-800 p-2.5">
                    ...
                    <div
                      className={`text-lg font-bold ${tractResult.unemploymentQualifies ? "text-red-400" : "text-white"}`}
                    >
                      {tractResult.unemploymentRate.toFixed(1)}%
                    </div>
                    {tractResult.unemploymentQualifies && (
                  ...
                  <div className="bg-gray-800 p-2.5">
                    ...
                    <div
                      className={`text-lg font-bold ${tractResult.severelyDistressed ? "text-purple-400" : "text-gray-500"}`}
                    >
                ...
                <div className="px-3 py-2 border-t border-gray-700 flex justify-between items-center">
                  ...
                  <span
                    className={`text-xs font-medium px-2 py-0.5 rounded ${tractResult.metroStatus === "Non-Metro" ? "bg-amber-900/50 text-amber-300" : "bg-gray-700 text-gray-300"}`}
                  >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400">
                      {sponsorDeals.length} deal
                      {sponsorDeals.length !== 1 ? "s" : ""}
                    </p>
                  </div>
                </div>
                <Link
                        ...
                        <div className="flex items-start justify-between gap-2">
                          <div className="min-w-0 flex-1">
                            <div className="flex items-center gap-2">
                              ...
                              <p
                                className={`text-sm font-medium truncate ${isSelected ? "text-indigo-300" : "text-white"}`}
                              >
                                {deal.projectName}
                              </p>
                            </div>
                          ...
                          <span
                            className={`flex-none px-2 py-0.5 text-xs rounded ${
                              deal.status === "draft"
          ...
          <div className="border-b border-gray-800">
            <div className="p-4 bg-gradient-to-r from-indigo-950/30 to-purple-950/30">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className={`relative w-11 h-6 rounded-full transition-colors ${autoMatchEnabled ? "bg-indigo-600" : "bg-gray-700"}`}
                >
                  <span
                    className={`absolute top-1 left-1 w-4 h-4 bg-white rounded-full transition-transform ${autoMatchEnabled ? "translate-x-5" : ""}`}
                  />
                </button>
              </div>
              {autoMatchEnabled && (
                <div className="mt-3 flex gap-2">
                  <span className="px-2 py-0.5 text-xs bg-indigo-900/50 text-indigo-300 rounded">
                    3-Deal Rule
                  </span>
            ...
            {viewMode === "sponsor" && autoMatchEnabled && (
              <div className="p-4 bg-gray-900/50">
                {selectedDealId &&
                sponsorDeals.find((d) => d.id === selectedDealId) ? (
                  <>
                    {(() => {
                      const selectedDeal = sponsorDeals.find(
                        (d) => d.id === selectedDealId,
                      )!;
                      return (
                        <div className="space-y-3">
                          <div className="flex items-center justify-between">
                          ...
                          <div className="p-3 bg-indigo-900/30 border border-indigo-700/50 rounded-lg">
                            ...
                            <div className="flex gap-2 mt-2">
                              ...
                              {selectedDeal.allocation > 0 && (
                                <span className="text-xs text-gray-500">
                                  $
                                  {(selectedDeal.allocation / 1000000).toFixed(
                                    1,
                                  )}
                                  M ask
                                </span>
                          ...
                          <div className="pt-3 border-t border-gray-800">
                            ...
                            <p className="text-xs text-gray-500">
                              CDEs ranked by match score appear on the map.
                  ...
                  <div className="text-center py-4">
                    ...
                    <p className="text-xs text-gray-600">
                      Choose a project to find matching CDEs & Investors
                    </p>
                  </div>
                )}
              </div>
            ...
            {viewMode === "cde" ? (
              ...
              <>
                <div className="p-3 bg-green-900/20 border border-green-800/30 rounded-lg mb-3">
                  ...
                  <p className="text-xs text-gray-400">
                    As a CDE, you deploy NMTC allocation to qualified projects
                  </p>
                </div>
                <label className="flex items-center gap-3 cursor-pointer group">
                  <input
                    type="checkbox"
                    checked={filters.creditTypes.includes("nmtc")}
                    onChange={() => toggleCreditType("nmtc")}
                ...
                <div className="pt-3 mt-3 border-t border-gray-800">
                  <label className="flex items-center gap-3 cursor-pointer group">
                    <input
                      type="checkbox"
                      checked={filters.includeStateCredits}
                      onChange={() =>
                        onFiltersChange({
                          ...filters,
                          includeStateCredits: !filters.includeStateCredits,
                        })
                      }
                      className="w-4 h-4 rounded border-gray-600 bg-gray-800 text-indigo-600"
                    />
                    <span className="text-sm text-gray-300 group-hover:text-white">
                      Include State NMTC Programs
                    </span>
                  </label>
                </div>
              </>
              ...
              <>
                ...
                {[
                  {
                    ...
                    desc: "New Markets Tax Credit",
                  },
                  {
                    key: "htc" as const,
                    label: "HTC",
                    color: "text-amber-600",
                    desc: "Historic Tax Credit",
                  },
                  ...
                  >
                    <input
                      ...
                      className={`w-4 h-4 rounded border-gray-600 bg-gray-800 ${color}`}
                    />
                    <div>
                      <span className="text-sm text-gray-300 group-hover:text-white">
                        {label}
                ...
                {[
                  {
                    ...
                    desc: "Low-Income Housing",
                  },
                  {
                    key: "oz" as const,
                    label: "OZ",
                    color: "text-blue-600",
                    desc: "Opportunity Zone",
                  },
                  {
                    key: "brownfield" as const,
                    label: "Brownfield",
                ...
                <div className="pt-3 mt-3 border-t border-gray-800">
                  ...
                  <p className="text-xs text-gray-500 ml-7 mt-1">
                    State NMTC, State HTC, State LIHTC
                  </p>
                </div>
              </>
            )}
        ...
        {matchCriteria && onMatchCriteriaChange && (
          <AccordionSection
            ...
            defaultOpen={true}
            badge={(() => {
              const count = Object.values(matchCriteria).filter(Boolean).length;
              return count > 0 ? (
                <span className="px-1.5 py-0.5 text-xs bg-emerald-900 text-emerald-300 rounded">
                  {count}
                </span>
              ) : undefined;
            })()}
        ...
        {viewMode === "sponsor" ? (
              ...
              <div className="space-y-3">
                <div className="p-3 bg-green-900/20 border border-green-800/30 rounded-lg">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="w-2 h-2 bg-green-500 rounded-full" />
                    <span className="text-sm font-medium text-green-400">
                      Find CDEs with Allocation
                    </span>
                  </div>
                  <p className="text-xs text-gray-400">
                    CDEs with remaining NMTC allocation for your project
                  </p>
                </div>
                    ...
                    <input
                      ...
                      value={
                        filters.cdeMinAllocationRemaining
                          ? filters.cdeMinAllocationRemaining / 1000000
                          : ""
                      }
                      onChange={(e) =>
                        onFiltersChange({
                          ...filters,
                          cdeMinAllocationRemaining: e.target.value
                ...
                <p className="text-xs text-gray-600">
                  Filter CDEs by minimum remaining allocation
                </p>
              </div>
            </AccordionSection>
            ...
            >
              <div className="space-y-2">
                ...
                >
                  ...
                  <p className="text-xs text-gray-400">
                    Browse investors seeking tax credit opportunities
                  </p>
                </Link>
                <div className="flex gap-3">
                  <div className="flex-1">
                    ...
                    <input
                      ...
                      placeholder="1"
                      value={
                        filters.allocationRequest.min
                          ? filters.allocationRequest.min / 1000000
                          : ""
                      }
                      onChange={(e) =>
                        onFiltersChange({
                  ...
                  <div className="flex-1">
                    ...
                    <input
                      ...
                      placeholder="50"
                      value={
                        filters.allocationRequest.max
                          ? filters.allocationRequest.max / 1000000
                          : ""
                      }
                      onChange={(e) =>
                        onFiltersChange({
          ...
          <AccordionSection
            ...
            defaultOpen={viewMode === "cde"}
          >
            <div className="space-y-2">
              {[
                {
                  key: "preDevelopment" as keyof FilterState,
                  label: "Pre-Development",
                  color: "text-yellow-600",
                },
                {
                  key: "seekingCapitalStack" as keyof FilterState,
                  label: "Seeking Capital Stack",
                  color: "text-orange-600",
                },
                {
                  key: "shovelReadyOnly" as keyof FilterState,
                  label: "Shovel Ready",
                  color: "text-green-600",
                },
                {
                  key: "seekingAllocation" as keyof FilterState,
                  label: "Seeking Allocation",
                  color: "text-indigo-600",
                },
                {
                  key: "inClosing" as keyof FilterState,
                  label: "In Closing",
                  color: "text-purple-600",
                },
              ].map(({ key, label, color }) => (
            ...
            <div className="space-y-2">
              <label className="flex items-center gap-3 cursor-pointer group">
                <input
                  ...
                  className="w-4 h-4 rounded border-gray-600 bg-gray-800 text-red-600"
                />
                <span className="text-sm text-gray-300 group-hover:text-white">
                  Severely Distressed Only
                </span>
              </label>
              <label className="flex items-center gap-3 cursor-pointer group">
                <input
              ...
              <label className="flex items-center gap-3 cursor-pointer group">
                <input
                  type="checkbox"
                  checked={filters.underservedStates}
                  onChange={() => toggleFilter("underservedStates")}
              ...
              <p className="text-xs text-gray-500 ml-7">
                Non-metro areas & Native American communities
              </p>
            </div>
          </AccordionSection>
        )}
          ...
          >
            <div className="flex gap-2">
              {[
                ...
                { value: "urban" as const, label: "Urban" },
                { value: "rural" as const, label: "Rural" },
              ].map(({ value, label }) => (
                <button
                  key={value}
                  onClick={() =>
                    onFiltersChange({ ...filters, areaType: value })
                  }
                  className={`flex-1 px-3 py-2 text-sm rounded-lg transition-colors ${filters.areaType === value ? "bg-indigo-600 text-white" : "bg-gray-800 text-gray-400 hover:text-white hover:bg-gray-700"}`}
                >
                  {label}
                </button>
              ))}
          ...
          <AccordionSection
            ...
            badge={
              filters.projectTypes.length > 0 ? (
                <span className="px-1.5 py-0.5 text-xs bg-indigo-900 text-indigo-300 rounded">
          ...
          >
            <div className="space-y-3">
              <label className="flex items-center gap-3 cursor-pointer group">
                <input
                  type="checkbox"
                  checked={filters.communityImpact.wealthCreation}
                  onChange={() =>
                    updateCommunityImpact({
                      wealthCreation: !filters.communityImpact.wealthCreation,
                    })
                  }
                  className="w-4 h-4 rounded border-gray-600 bg-gray-800 text-green-600"
                />
              ...
              <label className="flex items-center gap-3 cursor-pointer group">
                <input
                  type="checkbox"
                  checked={filters.communityImpact.productsServicesInLIC}
                  onChange={() =>
                    updateCommunityImpact({
                      productsServicesInLIC:
                        !filters.communityImpact.productsServicesInLIC,
                    })
                  }
                  className="w-4 h-4 rounded border-gray-600 bg-gray-800 text-blue-600"
                />
                <span className="text-sm text-gray-300 group-hover:text-white">
                  Products/Services in LIC
                </span>
              </label>
              <label className="flex items-center gap-3 cursor-pointer group">
                <input
                  type="checkbox"
                  checked={filters.communityImpact.productsServicesOutsideLIC}
                  onChange={() =>
                    updateCommunityImpact({
                      productsServicesOutsideLIC:
                        !filters.communityImpact.productsServicesOutsideLIC,
                    })
                  }
                  className="w-4 h-4 rounded border-gray-600 bg-gray-800 text-purple-600"
                />
                <span className="text-sm text-gray-300 group-hover:text-white">
                  Products/Services outside LIC
                </span>
              </label>
              <div className="pt-2 border-t border-gray-800">
                <label className="flex items-center gap-3 cursor-pointer group mb-2">
                  <input
                    type="checkbox"
                    checked={filters.communityImpact.jobCreation.enabled}
                    onChange={() =>
                      updateCommunityImpact({
                        jobCreation: {
                          ...filters.communityImpact.jobCreation,
                          enabled: !filters.communityImpact.jobCreation.enabled,
                        },
                      })
                    }
                    className="w-4 h-4 rounded border-gray-600 bg-gray-800 text-indigo-600"
                  ...
                  <div className="ml-7 flex gap-3">
                    <div className="flex-1">
                      ...
                      <input
                        ...
                        placeholder="Min"
                        value={
                          filters.communityImpact.jobCreation.fullTimeMin || ""
                        }
                        onChange={(e) =>
                          updateCommunityImpact({
                            jobCreation: {
                    ...
                    <div className="flex-1">
                      ...
                      <input
                        ...
                        placeholder="Min"
                        value={
                          filters.communityImpact.jobCreation.partTimeMin || ""
                        }
                        onChange={(e) =>
                          updateCommunityImpact({
                            jobCreation: {
          ...
          >
            <div className="space-y-3">
              <p className="text-xs text-gray-500 mb-2">
                Filter by QLICI allocation request size
              </p>
              <div className="flex gap-3">
                <div className="flex-1">
                  <label className="text-xs text-gray-500 block mb-1">
                  ...
                  <input
                    type="number"
                    placeholder="1000000"
                    value={filters.allocationRequest.min || ""}
                    onChange={(e) =>
                      onFiltersChange({
                        ...filters,
                        allocationRequest: {
                ...
                <div className="flex-1">
                  ...
                  <input
                    ...
                    value={filters.allocationRequest.max || ""}
                    onChange={(e) =>
                      onFiltersChange({
                        ...filters,
                        allocationRequest: {
              ...
              <p className="text-xs text-gray-600">
                Small deal funds typically $1-5M per QLICI
              </p>
            </div>
          </AccordionSection>
        )}
          ...
          >
            <div className="space-y-2">
              <label className="flex items-center gap-3 cursor-pointer group">
                <input
                  type="checkbox"
                  checked={filters.excludeRelatedParty}
                  onChange={() => toggleFilter("excludeRelatedParty")}
                  className="w-4 h-4 rounded border-gray-600 bg-gray-800 text-red-600"
                />
                <span className="text-sm text-gray-300 group-hover:text-white">
                  Exclude Related Party Deals
                </span>
              </label>
              <p className="text-xs text-gray-500 ml-7">
                Transactions with common ownership/management
              </p>
            </div>
          </AccordionSection>
        )}
